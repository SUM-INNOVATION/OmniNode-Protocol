...
The Stage 12.20–12.25 signed-integrity-evidence family (signed integrity baselines and diffs, local integrity-evidence bundles, and integrity-evidence chain reports) and the Stage 13 chain evidence-anchor path have been removed. They were local-only Ed25519 signing/anchoring layers stacked on top of the Stage 12.19 integrity diagnostics, which remain in place. The `state-integrity` and `state-integrity-diff` commands and the `diff_state_integrity_reports` library API are unaffected.
