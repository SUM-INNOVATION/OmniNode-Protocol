...
| **Phase 5 Stage 11b.1.a** — Bounded multi-framework halo2 reference scaffold (no halo2 circuit yet): `ModelFramework` enum (Rumus / PyTorch / TensorFlow / Caffe / FrameworkAgnostic), `ModelFormat::Halo2ReferenceMlp`, `ProofSystem::Stage11bHalo2Reference`, canonical bounded MLP spec (4→8→4 with ReLU, int16 fixed-point), pure-Rust canonical evaluator, `FrameworkManifest` schema, 5 committed framework manifests, cross-framework equivalence integration test. RUMUS shipped as fixture-only (no deterministic CPU fixed-point integer-dense path yet). **Zero prover deps, zero halo2 deps, zero operator-binary changes, Stage 11a + Stage 11b.0 fixtures byte-stable.** | `omni-zkml`, `crates/omni-proofs-halo2-reference/` (new) | **Complete** |
| Phase 5 Stage 11b.1.b — Halo2 circuit + verifier crate impl + frozen proof fixture + verifier-only CI gate + manual regen workflow | `crates/omni-proofs-halo2-reference/` (extends), `omni-node`, `.github/workflows` | Planned |
