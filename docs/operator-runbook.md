...
| **State-dir archive & forensic record** | 12.14–12.19 | `archive-session`, `restore-session-archive`, `state-integrity`, `plan-state-cleanup`, `apply-state-cleanup`, `restore-state-cleanup-quarantine`, `state-integrity-diff` | Capture and verify operator-local integrity diagnostics of state-dir contents. |
...
- **Nightly batch** (cron / systemd timer): capture a forensic-record baseline with Stage 12.16 `state-integrity --json-out`, and optionally diff it against a previous baseline with Stage 12.19 `state-integrity-diff --json-out`. Produces an operator-local integrity record per night.
...
### Stages 12.20–12.25 and Stage 13 — removed

The signed-integrity-evidence family (Stage 12.20–12.25: `sign-state-integrity-baseline`, `sign-state-integrity-diff`, `build-integrity-evidence-bundle`, `sign-integrity-evidence-bundle`, `verify-integrity-evidence-chain`, `sign-integrity-evidence-chain-report`, and their verify counterparts) and the Stage 13 chain evidence-anchor path (`operator evidence-anchor …`) have been removed. Operator-local integrity diagnostics remain: use Stage 12.16 `state-integrity` for a baseline scan and Stage 12.19 `state-integrity-diff` to diff a later scan against it.
