# OmniNode Protocol — Project Status & Delivery Record

Durable stage-by-stage delivery record and roadmap for OmniNode
Protocol. Formerly the bulk of `README.md`; extracted so `README.md`
can be a professional entrypoint and this doc can be the detailed
reference.

**Audience:** contributors and reviewers auditing what has actually
shipped, phase-by-phase and stage-by-stage. First-time operators or
integrators should start with [`README.md`](../README.md); they will
find pointers back to this doc when they need it.

**How to use:**

- The **Current Status** table below is the top-level roll-up per
  Phase / Stage. Everything below the table is prose that expands
  each stage in delivery order.
- Feature detail docs (e.g. [`production-mlp-proof.md`](production-mlp-proof.md))
  are the sources of truth for build/use recipes; this doc records
  the *shipping history* that produced those features.
- Immutable audit snapshots live at
  [`phase5-rc-audit.md`](phase5-rc-audit.md) (2026-05-21) and
  [`phase5-rc-audit-2026-06-24.md`](phase5-rc-audit-2026-06-24.md);
  they are point-in-time records and are not edited after landing.

**What this doc does NOT do:**

- It is **not** a chain-team review packet. For that see
  [`stage11d.2-review-packet-production-fixedpoint-mlp.md`](stage11d.2-review-packet-production-fixedpoint-mlp.md).
- It is **not** the operator runbook. For that see
  [`operator-runbook.md`](operator-runbook.md).
- It is **not** a promise of future scope beyond the "Planned" rows.
  Anything marked "Planned" is subject to explicit stage-plan review
...
| **Phase 5 Stage 10b** — Release artifact workflow + cosign keyless signing (delivered via Stage 15.2; first signed release `v0.1.0` cut, signed, and verified end-to-end via Stage 15.2A): `.github/workflows/release.yml` with hard tag/version match gate, two-variant matrix (`default` + `submit`, `x86_64-unknown-linux-gnu`), `SHA256SUMS` + `SHA256SUMS.sig` + `SHA256SUMS.cert` produced under Sigstore Fulcio + Rekor with no long-lived key, governance lock via cert identity regex pinned in `docs/stage15.2-release-artifact-workflow.md`, draft-on-tag-push with manual promotion gate. | `.github/workflows/release.yml`, `docs/stage15.2-release-artifact-workflow.md`, `docs/operator-runbook.md` | **Complete (verified via v0.1.0)** |
...
| **Phase 5 Stage 12.25** — Signed integrity-evidence chain reports. Local-only Ed25519-signed wrapper around a v1 `IntegrityEvidenceChainReport` plus a `sign-integrity-evidence-chain-report` CLI to produce one and a `verify-integrity-evidence-chain-report-signature` CLI to verify it — so operators can attest "this chain report came from a specific key at a specific time" without re-running Stage 12.24's verification gates. The signature verifier is **provenance-only**. **No new protocol envelope. No canonical-byte changes on Stage 12.0–12.24 surfaces. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATE_INTEGRITY_DIFF_SCHEMA_VERSION` / `SIGNED_BASELINE_SCHEMA_VERSION` / `SIGNED_INTEGRITY_DIFF_SCHEMA_VERSION` / `INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION` / `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION` / `INTEGRITY_EVIDENCE_CHAIN_REPORT_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** Stage 12.24's v1 `IntegrityEvidenceChainReport` is consumed UNCHANGED. New local-only schema `SIGNED_INTEGRITY_EVIDENCE_CHAIN_REPORT_SCHEMA_VERSION = 1`, independent of every existing schema constant — including `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION` and `INTEGRITY_EVIDENCE_CHAIN_REPORT_SCHEMA_VERSION`; v1 wrappers wrap v1 chain reports only. New `omni_contributor::signed_chain` module with `pub fn sign_integrity_evidence_chain_report(chain_report, signer_pubkey_hex, signer_role, signed_at_utc, sign_fn) -> Result<SignedIntegrityEvidenceChainReport, SignedIntegrityEvidenceChainReportError>` (signer-agnostic — the caller supplies a closure that computes the signature; refuses non-v1 embedded chain reports; **pre-sign validation is deliberately narrow** — only the embedded `chain_report.schema_version` is gated, matching the Stage 12.20/12.21/12.23 "sign these bytes" posture; an operator who hands the signer a chain report describing a failed verification gets a wrapper attesting to that failure verbatim), `pub fn verify_signed_integrity_evidence_chain_report(wrapper, expected_signer_pubkey_hex) -> Result<(), SignedIntegrityEvidenceChainReportError>` (two-step refusal: cheap `SignerPubkeyMismatch` pre-check first, then crypto verify; refuses non-v1 wrapper / non-v1 embedded chain report at the same gate), `pub fn signed_integrity_evidence_chain_report_signing_input(...)` (builds `SIGNED_INTEGRITY_EVIDENCE_CHAIN_REPORT_DOMAIN \|\| bincode1::serialize(canonical_body)` mirroring the Stage 12.0–12.23 envelope pattern), `pub fn read_signed_integrity_evidence_chain_report_from_path` + `pub fn write_signed_integrity_evidence_chain_report_atomic` (atomic temp+rename writer matching Stage 12.17/12.20/12.21/12.23). `BaselineSignerRole` is **reused verbatim** from Stage 12.20 — variants are role names, not artifact-type names, so the same taxonomy applies to baselines, diffs, bundles, AND chain reports alike. `SignedIntegrityEvidenceChainReport { schema_version, signed_at_utc, signer_pubkey_hex, signer_role, chain_report, signature_hex }` with `deny_unknown_fields` — embeds the full v1 `IntegrityEvidenceChainReport` so a captured wrapper is self-contained (no out-of-band chain-report file needed at verify time). New typed `SignedIntegrityEvidenceChainReportError { UnsupportedSchemaVersion, UnsupportedChainReportSchemaVersion, Canonical, Signing, SignatureMismatch, SignerPubkeyMismatch, Io, MalformedJson }` — mirrors Stage 12.20/12.21/12.23 shape verbatim. New canonical-bytes constant `pub const SIGNED_INTEGRITY_EVIDENCE_CHAIN_REPORT_DOMAIN: &[u8] = b"OMNINODE-CONTRIBUTOR-SIGNED-INTEGRITY-EVIDENCE-CHAIN-REPORT:v1:"` — distinct from `SIGNED_BASELINE_DOMAIN`, `SIGNED_INTEGRITY_DIFF_DOMAIN`, and `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_DOMAIN` so the four signed-artifact families can't be cross-substituted. **Safety contract**: trust is opt-in at the verifier (`--expected-signer-pubkey-hex` is the operator-supplied anchor — the wrapper's embedded pubkey is forensic context only); two-step refusal order means malicious wrappers with forged pubkeys never burn crypto cycles; canonical body covers every field of the wrapper EXCEPT `signature_hex`, so tampering `signed_at_utc` / `signer_role` / `signer_pubkey_hex` / any field of the embedded chain report (including Stage 12.24's minimal `bundle_signer_role` / `bundle_signer_pubkey_hex` fields, every `BundleEntryVerifyOutcome.outcome` enum payload, every `ChainChildEntryOutcome.signature_outcome` enum payload, and the summary counts) all trip `SignatureMismatch`; **the wrapper transitively pins the entire forensic record of one chain verification under a single Ed25519 signature**; determinism is pinned by regression (same chain report + seed + `signed_at_utc` → byte-identical wrapper); read-only — neither signing nor verifying opens a state-store, the signing CLI writes only the operator-named `--out` file (atomic temp+rename), and the verifier writes nothing. **New CLI subcommand `omni-node operator contributor sign-integrity-evidence-chain-report`** with `--chain-report-in <path>`, `--signer-seed <path>` (32-byte raw Ed25519 seed), `--signer-role operator\|contributor\|dispatcher\|coordinator`, `--out <path>`, `--format events\|json\|pretty`. Read-only on the state-store side; auto-prune flag deliberately absent (Stage 12.16/12.17/12.18/12.19/12.20/12.21/12.22/12.23 precedent), pinned by clap regression. **New CLI subcommand `omni-node operator contributor verify-integrity-evidence-chain-report-signature`** with `--signed-chain-report <path>`, `--expected-signer-pubkey-hex <hex>` (64-char lowercase Ed25519 pubkey trust anchor), `--format events\|json\|pretty`. Opens NO state-store; auto-prune flag deliberately absent, pinned by clap regression. **No `--base-dir` flag** — that's a Stage 12.22 concept; this verifier attests to chain-report JSON bytes only. **No `--chain-report-out` extraction flag** in v1 — operators can extract via `--format json` + `jq`. Verifier JSON renderer emits a **compact metadata view** — wrapper-level fields plus embedded chain-report metadata (schema, generated_at_utc, signed_bundle_path, effective_base_dir, bundle_signer_role, bundle_signer_pubkey_hex) plus summary counts (`bundle_byte_counts`, `child_counts`) — does NOT re-print the full embedded per-entry or per-child lists (operators who want them have the wrapper on disk). Closed-set events: `event=signed_integrity_evidence_chain_report_signing_started`, `event=signed_integrity_evidence_chain_report_written`, `event=signed_integrity_evidence_chain_report_sign_failed reason=<closed-tag>` (closed tags: `io` / `malformed_json` / `signing` / `unsupported_chain_report_schema_version` / `canonical` — emitted on every signer failure path), `event=signed_integrity_evidence_chain_report_verify_started signed_chain_report=<path>` (fires FIRST, before any FS IO, matching the Stage 12.22/12.23/12.24 review-fix posture so operators always see a start event even on missing/malformed wrappers), `event=signed_integrity_evidence_chain_report_verify_ok`, `event=signed_integrity_evidence_chain_report_verify_failed reason=<closed-tag>` (closed tags: `unsupported_schema_version` / `unsupported_chain_report_schema_version` / `signer_pubkey_mismatch` / `signature_mismatch` / `signing` / `canonical` / `io` / `malformed_json` — wrapper-read failures distinguish `reason=io` from `reason=malformed_json` via the same closed mapper). Both event families single-source through the same `signed_chain_report_reason_tag` mapper so signer and verifier can't diverge — pinned by regression. **Out of scope (v1)**: no re-running of Stage 12.24 gates (signature verifier is provenance-only — operators run `verify-integrity-evidence-chain` directly if they want to re-verify), no recursive signature verification of embedded signed-baseline / signed-diff / signed-bundle artifacts referenced by the chain report (Stage 12.20/12.21/12.23's job), no new `IntegrityEvidenceChainReport` schema fields, no `--base-dir` flag, no `--chain-report-out` extraction flag, no wrapper expiry / `valid_through_utc`, no multi-signature / threshold signatures, no wrapper encryption, no revocation list, no external signed-chain-report registry / publication. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.24 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 19 signed-integrity-evidence-chain-report integration tests cover the full accept/refuse matrix (sign + verify happy path, all four `BaselineSignerRole` variants round-trip, `SignerPubkeyMismatch` cheap pre-check before crypto, signature tamper → `SignatureMismatch`, **embedded chain-report scalar field tamper → `SignatureMismatch`** proving scalar fields are in the canonical body, **embedded chain-report child-signature enum payload tamper → `SignatureMismatch`** proving nested enum payloads are in the canonical body — the bincode-1 stability surface most likely to break inadvertently, `signer_role` tamper → `SignatureMismatch`, `signed_at_utc` tamper → `SignatureMismatch`, **Stage 12.24 `bundle_signer_pubkey_hex` tamper → `SignatureMismatch`** proving the minimal Stage 12.24 signer-metadata fields are in the canonical body, wrapper schema v2 → `UnsupportedSchemaVersion`, embedded chain-report schema v2 → `UnsupportedChainReportSchemaVersion`, sign refuses a non-v1 input chain report, JSON round-trip preserves signature, **determinism — same chain report + seed + `signed_at_utc` produces byte-identical wrapper**, canonical signing-input byte stability + domain prefix `OMNINODE-CONTRIBUTOR-SIGNED-INTEGRITY-EVIDENCE-CHAIN-REPORT:v1:`, `write_signed_integrity_evidence_chain_report_atomic` round-trips via disk, **six-stage composition** — Stage 12.20 sign baseline → 12.21 sign diff → 12.22 build bundle → 12.23 sign bundle → 12.24 chain verify → 12.25 sign chain report → 12.25 verify chain-report signature; the strongest "the whole stack composes" regression, **wrapper read missing-file → `Io { NotFound }` (NOT `MalformedJson`)** — pins the library variant the CLI's `reason=io` tag depends on, **wrapper read present-but-unparseable → `MalformedJson` (NOT `Io`)** — counterpart pin). 2 new CLI clap regressions pin both flag matrices: `sign-integrity-evidence-chain-report` (all 4 roles, `--format` closed enum + unknown-format refusal, required-flag refusal sweep, unknown-role refusal, deliberate rejection of `--no-prune-state-on-start`) and `verify-integrity-evidence-chain-report-signature` (required `--signed-chain-report` + `--expected-signer-pubkey-hex`, `--format` closed enum + unknown-format refusal, deliberate rejection of `--no-prune-state-on-start`). 1 new closed reason-tag regression pins the `signed_chain_report_reason_tag` mapper against every `SignedIntegrityEvidenceChainReportError` variant by name. New operator-runbook section walks signing → trust anchor capture → archival verification + composition with Stage 12.20/12.21/12.22/12.23/12.24 (the full **six-stage forensic chain terminating in Stage 12.25**) + per-`SignedIntegrityEvidenceChainReportError` triage. | `crates/omni-contributor/src/signed_chain.rs` (new — `sign_integrity_evidence_chain_report` / `verify_signed_integrity_evidence_chain_report` / `SignedIntegrityEvidenceChainReport` / `signed_integrity_evidence_chain_report_signing_input` / `read_signed_integrity_evidence_chain_report_from_path` / `write_signed_integrity_evidence_chain_report_atomic` / `SIGNED_INTEGRITY_EVIDENCE_CHAIN_REPORT_SCHEMA_VERSION`); extended `crates/omni-contributor/src/canonical.rs` (new `SIGNED_INTEGRITY_EVIDENCE_CHAIN_REPORT_DOMAIN`), `crates/omni-contributor/src/error.rs` (new `SignedIntegrityEvidenceChainReportError`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `sign-integrity-evidence-chain-report` + `verify-integrity-evidence-chain-report-signature` subcommands + dispatch + run fns + 6 renderers + 2 clap regressions + 1 reason-tag regression); `crates/omni-contributor/tests/signed_integrity_evidence_chain_report.rs` (new — 19 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.25 section), `docs/operator-runbook.md` (Stage 12.25 sign + verify workflow section) | **Complete** |
| **Phase 5 Stage 12.24** — Local integrity-evidence chain verification. New `verify-integrity-evidence-chain` CLI that composes Stage 12.20–12.23 verifiers into a single read-only operation: (1) signed-bundle signature gate (Stage 12.23), (2) bundle byte verification (Stage 12.22), (3) optional per-signed-child signature verification (Stage 12.20 / 12.21) — so an operator runs ONE command to walk the full trust chain rooted at a signed bundle, with explicit `Skipped` outcomes when an expected pubkey flag is omitted. **No new protocol envelope. No canonical-byte changes on Stage 12.0–12.23 surfaces. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATE_INTEGRITY_DIFF_SCHEMA_VERSION` / `SIGNED_BASELINE_SCHEMA_VERSION` / `SIGNED_INTEGRITY_DIFF_SCHEMA_VERSION` / `INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION` / `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change. No new canonical-bytes constant. No new domain separator. No new signing.** Stage 12.22's `IntegrityEvidenceBundle` and Stage 12.23's `SignedIntegrityEvidenceBundle` are consumed UNCHANGED. New local-only schema `INTEGRITY_EVIDENCE_CHAIN_REPORT_SCHEMA_VERSION = 1`, independent of every existing schema constant; v1 chain reports describe one bundle level only — no recursion. New `omni_contributor::integrity_evidence_chain` module with `pub fn verify_integrity_evidence_chain(opts: &ChainVerifyOptions) -> Result<IntegrityEvidenceChainReport, ChainVerifyError>` (pure composition: read + verify Stage 12.23 wrapper FIRST → short-circuit on envelope refusal → Stage 12.22 byte verify → short-circuit on envelope refusal → collect-all over signed children) + `pub fn write_integrity_evidence_chain_report_atomic(report, out)` (atomic temp+rename writer for the optional `--json-out` mirror; best-effort posture). Closed `ChainStepOutcome { Ok, Skipped, Failed { reason, detail } }` with `snake_case` serde tagging via `#[serde(tag = "status")]` — **`Skipped`** is NOT a pass; surfaces in the report so operators see exactly what wasn't verified. `ChainChildEntryOutcome { artifact_kind, path, resolved_path, signature_outcome }` with `deny_unknown_fields`; only emitted for `signed_state_integrity_baseline` / `signed_state_integrity_diff` entries (other kinds are bundle-byte-only). `IntegrityEvidenceChainReport { schema_version, generated_at_utc, omni_contributor_version, signed_bundle_path, effective_base_dir, bundle_signature, bundle_signer_role, bundle_signer_pubkey_hex, bundle_byte_verify, child_signatures, counts_child_ok, counts_child_skipped, counts_child_failed }` with `deny_unknown_fields` — embeds the **full Stage 12.22 `BundleVerifyReport`** so operators get every per-entry outcome without re-running; **does NOT embed the full `SignedIntegrityEvidenceBundle` wrapper** to keep the report compact. The verified wrapper's `signer_role` + `signer_pubkey_hex` flow into the report as the minimal `bundle_signer_role` / `bundle_signer_pubkey_hex` fields so the CLI's `..._signed_bundle_ok` event can carry signer identity (the start line only carries `expected_*_pubkey=set` markers and deliberately doesn't leak the pubkey). `IntegrityEvidenceChainReport::all_required_ok()` helper for exit-code decisions: true iff `bundle_signature == Ok` AND `bundle_byte_verify.all_ok()` AND `counts_child_failed == 0` — **`Skipped` child outcomes DON'T fail** (operator-choice informational). New typed `ChainVerifyError { UnsupportedChainSchemaVersion, SignedBundle(#[from] SignedIntegrityEvidenceBundleError), BundleByte(#[from] EvidenceBundleError), Io, MalformedJson }` — the last two only fire on the optional `--json-out` write. Two public per-child reason-tag mappers (`baseline_child_reason_tag` / `diff_child_reason_tag`) for `SignedBaselineError` / `SignedIntegrityDiffError`, pinned by regression. **Safety contract**: outermost gate is short-circuit (if Stage 12.23 wrapper fails, chain stops with no per-entry / per-child events); bundle byte verify runs after signed-bundle OK; child-signature verify is collect-all (per-entry bundle-byte failures DON'T stop child verifies — a `HashMismatch` on bytes plus a separately-verifiable signature on those bytes are independent forensic facts); omitted child anchor records `Skipped`, never silent pass; single trust anchor per child kind in v1; no recursive chain walking; no re-hashing of artifact files during child signature verify (Stage 12.20/12.21 verifiers cover canonical body bytes via `bincode1::serialize` of the typed struct); read-only across the whole chain (only write is optional `--json-out` atomic temp+rename); determinism pinned by regression (same inputs + same `now_utc` + same on-disk files → byte-identical report). **New CLI subcommand `omni-node operator contributor verify-integrity-evidence-chain`** with `--signed-bundle <path>` (required), `--expected-bundle-signer-pubkey-hex <hex>` (required), `--expected-baseline-signer-pubkey-hex <hex>` (optional gate; omission → baseline children record `Skipped`), `--expected-diff-signer-pubkey-hex <hex>` (optional gate; omission → diff children record `Skipped`), `--base-dir <path>` (optional Stage 12.22 override), `--json-out <path>` (optional best-effort report mirror; failure logs warn event, doesn't change exit), `--format events\|json\|pretty`. Opens NO state-store; auto-prune flag deliberately absent, pinned by clap regression. No clap-level pubkey hex validation (consistent with Stage 12.20–12.23). Closed-set events: `event=integrity_evidence_chain_verify_started signed_bundle=<...>[ base_dir_override=<...>][ expected_baseline_pubkey=set][ expected_diff_pubkey=set]` (fires FIRST, before any FS IO, matching the Stage 12.22/12.23 review-fix posture; `set` / absent markers let log scrapers see exactly which optional gates were enabled without leaking the pubkey hex); per-step `event=integrity_evidence_chain_signed_bundle_ok signer_role=<...> signer_pubkey=<...>` (signer fields sourced from the chain report's `bundle_signer_role` / `bundle_signer_pubkey_hex` so the event stream stays self-describing on signer identity) / `event=integrity_evidence_chain_bundle_byte_resolved effective_base_dir=<...>` / per-entry `event=integrity_evidence_chain_bundle_byte_entry_{ok|size_mismatch|hash_mismatch|not_found|read_error}` / per-child `event=integrity_evidence_chain_child_{ok|skipped|failed}`; summary `event=integrity_evidence_chain_verify_summary bundle_signature=ok bundle_byte_counts={...} child_counts={ok=<N> skipped=<N> failed=<N>}`; envelope-level failure `event=integrity_evidence_chain_verify_failed reason=<closed-tag>` + nonzero exit. Closed envelope-tag taxonomy uses **`signed_bundle_*` / `bundle_byte_*` prefixes** so the three signed-artifact families can't be cross-substituted (e.g. `signed_bundle_signer_pubkey_mismatch`, `bundle_byte_effective_base_dir_not_found`); per-child reason tags are unprefixed because the surrounding `kind=...` field disambiguates. **Exit policy**: 0 iff `bundle_signature == Ok` AND `bundle_byte_verify.all_ok()` AND `counts_child_failed == 0`; `Skipped` child outcomes DON'T fail exit; optional `--json-out` write failure DOESN'T affect exit (best-effort). **Out of scope (v1)**: no new signing (chain report is plain JSON; no Ed25519 on the chain artifact itself — natural follow-on would be a sibling signed-chain-report wrapper), no recursive chain walking, no new `BundleArtifactKind` variant, no multi-anchor child verification, no semantic JSON validation of raw unsigned artifacts (`state_integrity_report`, `cleanup_report`, etc.), no expiry / `valid_through_utc`. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.23 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 16 integrity-evidence-chain integration tests cover the full accept/refuse matrix (happy path with all three anchors → all children Ok, skip on omitted child anchors → children record `Skipped`, signed-bundle pubkey mismatch → `ChainVerifyError::SignedBundle` envelope refusal with no bundle-byte or child events, **bundle-byte hash_mismatch on a signed child STILL runs child signature verify proving independent forensic facts**, bundle-byte effective base_dir absent → `ChainVerifyError::BundleByte(EffectiveBaseDirNotFound)` with no child events, bundle-byte traversal path in embedded bundle → `ChainVerifyError::BundleByte(InvalidRelativePath)` BEFORE per-entry walk, child baseline signer mismatch → `Failed { reason: "signer_pubkey_mismatch" }` with OTHER children still walked, **child file missing yields both bundle-byte `NotFound` AND child `Failed { reason: "io" }` recorded** proving independent views, child malformed JSON → `Failed { reason: "malformed_json" }`, all-Skipped scenario when no anchors → all children Skipped + exit 0, bundle with zero signed children → child counts all zero + anchors silently unused, **determinism — same inputs + same `now_utc` produce byte-identical report**, JSON round-trip preserves report, `all_required_ok()` is true with skipped children only, **verified Stage 12.23 wrapper's `signer_role` + `signer_pubkey_hex` flow into the chain report as `bundle_signer_role` / `bundle_signer_pubkey_hex` — pins the contract the CLI's `..._signed_bundle_ok` event depends on**, `write_integrity_evidence_chain_report_atomic` round-trips via disk). 1 new CLI clap regression pins the `verify-integrity-evidence-chain` flag matrix (required `--signed-bundle` + `--expected-bundle-signer-pubkey-hex`, 4 optional flags, `--format` closed enum + unknown-format refusal, deliberate rejection of `--no-prune-state-on-start`). 1 new closed reason-tag regression pins the per-child `baseline_child_reason_tag` / `diff_child_reason_tag` mappers against every `SignedBaselineError` / `SignedIntegrityDiffError` variant by name — future variants without a mapping break the closed-set contract loudly at compile/test time. New operator-runbook section walks the four-trust-path forensic chain (Stage 12.20 → 12.21 → 12.22 → 12.23 → 12.24 single-command equivalent) with worked examples for "verify everything" / "verify only the bundle anchor" / "verify bundle + diff but not baseline" / relocated tree + `--json-out`, plus per-envelope and per-child reason-tag triage. | `crates/omni-contributor/src/integrity_evidence_chain.rs` (new — `verify_integrity_evidence_chain` / `write_integrity_evidence_chain_report_atomic` / `IntegrityEvidenceChainReport` / `ChainStepOutcome` / `ChainChildEntryOutcome` / `ChainVerifyOptions` / `baseline_child_reason_tag` / `diff_child_reason_tag` / `INTEGRITY_EVIDENCE_CHAIN_REPORT_SCHEMA_VERSION`); extended `crates/omni-contributor/src/error.rs` (new `ChainVerifyError`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `verify-integrity-evidence-chain` subcommand + dispatch + run fn + 3 renderers + 1 clap regression + 1 reason-tag regression); `crates/omni-contributor/tests/integrity_evidence_chain.rs` (new — 15 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.24 section), `docs/operator-runbook.md` (Stage 12.24 chain verification walkthrough section) | **Complete** |
| **Phase 5 Stage 12.23** — Signed integrity evidence bundles. Local-only Ed25519-signed wrapper around a v1 `IntegrityEvidenceBundle` plus a `sign-integrity-evidence-bundle` CLI to produce one and a `verify-integrity-evidence-bundle-signature` CLI to verify it — so operators can attest "this bundle came from a specific key at a specific time" while keeping byte-fingerprint verification (Stage 12.22 `verify-integrity-evidence-bundle`) as a separate step. **No new protocol envelope. No canonical-byte changes on Stage 12.0–12.22 surfaces. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATE_INTEGRITY_DIFF_SCHEMA_VERSION` / `SIGNED_BASELINE_SCHEMA_VERSION` / `SIGNED_INTEGRITY_DIFF_SCHEMA_VERSION` / `INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** Stage 12.22's v1 `IntegrityEvidenceBundle` is consumed UNCHANGED. New local-only schema `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION = 1`, independent of every existing schema constant — including `SIGNED_BASELINE_SCHEMA_VERSION` and `SIGNED_INTEGRITY_DIFF_SCHEMA_VERSION`; v1 wrappers wrap v1 bundles only. New `omni_contributor::signed_bundle` module with `pub fn sign_integrity_evidence_bundle(bundle, signer_pubkey_hex, signer_role, signed_at_utc, sign_fn) -> Result<SignedIntegrityEvidenceBundle, SignedIntegrityEvidenceBundleError>` (signer-agnostic — the caller supplies a closure that computes the signature; refuses non-v1 embedded bundles; **pre-sign validation is deliberately narrow** — only the embedded `bundle.schema_version` is gated, matching the Stage 12.20/12.21 "sign these bytes" posture), `pub fn verify_signed_integrity_evidence_bundle(wrapper, expected_signer_pubkey_hex) -> Result<(), SignedIntegrityEvidenceBundleError>` (two-step refusal: cheap `SignerPubkeyMismatch` pre-check first, then crypto verify; refuses non-v1 wrapper / non-v1 embedded bundle at the same gate), `pub fn signed_integrity_evidence_bundle_signing_input(...)` (builds `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_DOMAIN \|\| bincode1::serialize(canonical_body)` mirroring the Stage 12.0–12.21 envelope pattern), `pub fn read_signed_integrity_evidence_bundle_from_path` + `pub fn write_signed_integrity_evidence_bundle_atomic` (atomic temp+rename writer matching Stage 12.17/12.20/12.21). `BaselineSignerRole` is **reused verbatim** from Stage 12.20 — variants are role names, not artifact-type names, so the same taxonomy applies to baselines, diffs, AND bundles alike. `SignedIntegrityEvidenceBundle { schema_version, signed_at_utc, signer_pubkey_hex, signer_role, bundle, signature_hex }` with `deny_unknown_fields` — embeds the full v1 `IntegrityEvidenceBundle` so a captured wrapper is self-contained (no out-of-band bundle file needed at verify time). New typed `SignedIntegrityEvidenceBundleError { UnsupportedSchemaVersion, UnsupportedBundleSchemaVersion, Canonical, Signing, SignatureMismatch, SignerPubkeyMismatch, Io, MalformedJson }` — mirrors Stage 12.20/12.21 shape verbatim. New canonical-bytes constant `pub const SIGNED_INTEGRITY_EVIDENCE_BUNDLE_DOMAIN: &[u8] = b"OMNINODE-CONTRIBUTOR-SIGNED-INTEGRITY-EVIDENCE-BUNDLE:v1:"` — distinct from `SIGNED_BASELINE_DOMAIN` and `SIGNED_INTEGRITY_DIFF_DOMAIN` so the three signed-artifact families can't be cross-substituted. **Safety contract**: trust is opt-in at the verifier (`--expected-signer-pubkey-hex` is the operator-supplied anchor — the wrapper's embedded pubkey is forensic context only); two-step refusal order means malicious wrappers with forged pubkeys never burn crypto cycles; canonical body covers every field of the wrapper EXCEPT `signature_hex`, so tampering `signed_at_utc` / `signer_role` / `signer_pubkey_hex` / any field of the embedded bundle (including any per-entry `blake3_hex`) all trip `SignatureMismatch`; **because the bundle already records per-entry BLAKE3 hashes, the signed wrapper transitively pins the entire artifact tree without re-embedding any artifact bytes**; determinism is pinned by regression (same bundle + seed + `signed_at_utc` → byte-identical wrapper); read-only — neither signing nor verifying opens a state-store, the signing CLI writes only the operator-named `--out` file (atomic temp+rename), and the verifier writes nothing. **New CLI subcommand `omni-node operator contributor sign-integrity-evidence-bundle`** with `--bundle-in <path>`, `--signer-seed <path>` (32-byte raw Ed25519 seed), `--signer-role operator\|contributor\|dispatcher\|coordinator`, `--out <path>`, `--format events\|json\|pretty`. Read-only on the state-store side; auto-prune flag deliberately absent (Stage 12.16/12.17/12.18/12.19/12.20/12.21/12.22 precedent), pinned by clap regression. **New CLI subcommand `omni-node operator contributor verify-integrity-evidence-bundle-signature`** with `--signed-bundle <path>`, `--expected-signer-pubkey-hex <hex>` (64-char lowercase Ed25519 pubkey trust anchor), `--format events\|json\|pretty`. Opens NO state-store; auto-prune flag deliberately absent, pinned by clap regression. **No `--base-dir` flag** — that's a Stage 12.22 concept; this verifier attests to bundle JSON bytes only. Verifier JSON renderer emits a **compact metadata view** (`schema_version`, `signed_at_utc`, `signer_role`, `signer_pubkey_hex`, `signature_hex`, plus embedded `bundle.schema_version` / `generated_at_utc` / `omni_contributor_version` / `label` / `base_dir` / `entries.len()`) rather than re-printing the full embedded bundle's entries — verification's job is to attest authenticity, not to mirror bytes. Closed-set events: `event=signed_integrity_evidence_bundle_signing_started`, `event=signed_integrity_evidence_bundle_written`, `event=signed_integrity_evidence_bundle_sign_failed reason=<closed-tag>` (closed tags: `io` / `malformed_json` / `signing` / `unsupported_bundle_schema_version` / `canonical` — emitted on every signer failure path: bundle-in read, bundle-in parse, seed load, library refusal, atomic write), `event=signed_integrity_evidence_bundle_verify_started signed_bundle=<path>` (fires FIRST, before any FS IO, matching the Stage 12.22 review-fix posture so operators always see a start event even on missing/malformed wrappers), `event=signed_integrity_evidence_bundle_verify_ok`, `event=signed_integrity_evidence_bundle_verify_failed reason=<closed-tag>` (closed tags: `unsupported_schema_version` / `unsupported_bundle_schema_version` / `signer_pubkey_mismatch` / `signature_mismatch` / `signing` / `canonical` / `io` / `malformed_json` — bundle-read failures distinguish `reason=io` from `reason=malformed_json`). Both event families single-source through the same closed-tag mapper so signer and verifier can't diverge — pinned by `signed_bundle_reason_tag_covers_closed_set` regression. Any `SignedIntegrityEvidenceBundleError` exits nonzero. **Out of scope (v1)**: no re-hashing of bundled artifact files (Stage 12.22's job), no semantic JSON validation of bundled artifacts, no recursive signature verification of embedded `signed_state_integrity_baseline` / `signed_state_integrity_diff` entries (Stage 12.20/12.21's job), no pre-sign Stage 12.22 envelope-level validation (signer's contract is "sign these bytes"), no `--base-dir` flag, no `--bundle-out` extraction flag, no wrapper expiry / `valid_through_utc`, no multi-signature / threshold signatures, no wrapper encryption, no revocation list, no external signed-bundle registry / publication, no `IntegrityEvidenceBundle` schema change. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.22 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 17 signed-integrity-evidence-bundle integration tests cover the full accept/refuse matrix (sign + verify happy path, all four `BaselineSignerRole` variants round-trip, `SignerPubkeyMismatch` cheap pre-check before crypto, signature tamper → `SignatureMismatch`, **embedded-bundle tamper → `SignatureMismatch`** proving every byte of the bundle including per-entry hashes is in the canonical body, `signer_role` tamper → `SignatureMismatch`, `signed_at_utc` tamper → `SignatureMismatch`, wrapper schema v2 → `UnsupportedSchemaVersion`, embedded bundle schema v2 → `UnsupportedBundleSchemaVersion`, sign refuses a non-v1 input bundle, JSON round-trip preserves signature, **determinism — same bundle + seed + `signed_at_utc` produces byte-identical wrapper**, canonical signing-input byte stability + domain prefix `OMNINODE-CONTRIBUTOR-SIGNED-INTEGRITY-EVIDENCE-BUNDLE:v1:`, `write_signed_integrity_evidence_bundle_atomic` round-trips via disk, **composition with Stage 12.22** — build → sign → verify signature → verify bundle bytes (all four pass without re-hashing artifact files during signature verification), **wrapper read missing-file → `Io { NotFound }` (NOT `MalformedJson`)** — pins the library variant the CLI's `reason=io` tag depends on, **wrapper read present-but-unparseable → `MalformedJson` (NOT `Io`)** — counterpart pin). 2 new CLI clap regressions pin both flag matrices: `sign-integrity-evidence-bundle` (all 4 roles, `--format` closed enum + unknown-format refusal, required-flag refusal sweep, unknown-role refusal, deliberate rejection of `--no-prune-state-on-start`) and `verify-integrity-evidence-bundle-signature` (required `--signed-bundle` + `--expected-signer-pubkey-hex`, `--format` closed enum + unknown-format refusal, deliberate rejection of `--no-prune-state-on-start`). New operator-runbook section walks signing → trust anchor capture → archival verification + composition with Stage 12.20/12.21/12.22 (the full four-trust-path forensic chain: per-artifact signatures, bundle-byte verify, bundle signature verify) + per-`SignedIntegrityEvidenceBundleError` triage. | `crates/omni-contributor/src/signed_bundle.rs` (new — `sign_integrity_evidence_bundle` / `verify_signed_integrity_evidence_bundle` / `SignedIntegrityEvidenceBundle` / `signed_integrity_evidence_bundle_signing_input` / `read_signed_integrity_evidence_bundle_from_path` / `write_signed_integrity_evidence_bundle_atomic` / `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION`); extended `crates/omni-contributor/src/canonical.rs` (new `SIGNED_INTEGRITY_EVIDENCE_BUNDLE_DOMAIN`), `crates/omni-contributor/src/error.rs` (new `SignedIntegrityEvidenceBundleError`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `sign-integrity-evidence-bundle` + `verify-integrity-evidence-bundle-signature` subcommands + dispatch + run fns + 6 renderers + 2 clap regressions); `crates/omni-contributor/tests/signed_integrity_evidence_bundle.rs` (new — 17 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.23 section), `docs/operator-runbook.md` (Stage 12.23 sign + verify workflow section) | **Complete** |
| **Phase 5 Stage 12.22** — Local integrity evidence bundles. Local-only `IntegrityEvidenceBundle` manifest that ties together a chosen set of Stage 12.16–12.21 audit artifacts by `(artifact_kind, base-dir-relative path, byte_len, blake3_hex)` under a single `base_dir`, plus a `build-integrity-evidence-bundle` CLI to assemble one and a `verify-integrity-evidence-bundle` CLI to re-hash + collect outcomes — so an operator can hand a single tamper-evident forensic record archivable as an evidence artifact. **No new protocol envelope. No canonical-byte changes on Stage 12.0–12.21 surfaces. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATE_INTEGRITY_DIFF_SCHEMA_VERSION` / `SIGNED_BASELINE_SCHEMA_VERSION` / `SIGNED_INTEGRITY_DIFF_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change. No new canonical-bytes constant. No new domain separator.** New local-only schema `INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION = 1`, independent of every existing schema constant; v1 bundles describe v1 artifacts only. New `omni_contributor::integrity_evidence_bundle` module with `pub fn build_integrity_evidence_bundle(inputs, opts) -> Result<IntegrityEvidenceBundle, EvidenceBundleError>` (fail-fast builder; reads each input, computes `blake3::hash`, refuses on duplicate `(artifact_kind, path)`, oversized entries, and over-cap bundles), `pub fn verify_integrity_evidence_bundle(bundle, opts) -> Result<BundleVerifyReport, EvidenceBundleError>` (collect-all verifier; every entry gets a `BundleEntryOutcome`; the verifier never short-circuits per entry; envelope-level failures `UnsupportedSchemaVersion` / `EffectiveBaseDirNotFound` refuse with a typed error and no per-entry walk), `pub fn read_integrity_evidence_bundle_from_path` / `pub fn write_integrity_evidence_bundle_atomic` (atomic temp+rename writer matching Stage 12.17/12.20/12.21). Closed `BundleArtifactKind { StateIntegrityReport, SignedStateIntegrityBaseline, StateIntegrityDiffReport, SignedStateIntegrityDiff, StateCleanupPlan, CleanupReport, QuarantineManifest, QuarantineRestoreReport, ArchiveManifest, Other }` with `as_str()` / `from_wire_tag()` / `Display` and `snake_case` serde — only kinds that correspond to a file-emitting CLI surface today are represented; `Other` is the explicit escape hatch. `IntegrityEvidenceBundle { schema_version, generated_at_utc, omni_contributor_version, label?, notes?, base_dir, entries }` with `deny_unknown_fields` — entries sorted by `(artifact_kind.as_str(), path)` for byte-stable JSON. `BundleEntry { artifact_kind, path, bytes, blake3_hex }` with `deny_unknown_fields`; `path` is forward-slash-normalized relative to `base_dir`. Closed `BundleEntryOutcome { Ok, SizeMismatch { expected, got }, HashMismatch { expected, got }, NotFound, ReadError { detail } }` with `snake_case` serde tagging. New typed `EvidenceBundleError { UnsupportedSchemaVersion, EmptyBundle, DuplicateEntry, TooManyEntries, BundleLabelTooLong, NotesTooLong, EntryTooLarge, EntryNotFound, PathOutsideBaseDir, InvalidRelativePath, BaseDirInvalid, EffectiveBaseDirNotFound, NonUtf8Path, Io, MalformedJson }`. **Strict relative-path validator** `validate_relative_entry_path(&str)` runs on every recorded entry path at BOTH build time (after `--include` resolution; on relative inputs the raw UTF-8 string is validated BEFORE forward-slash normalization so backslashes refuse explicitly) AND verify envelope-level (walking `bundle.entries[].path` BEFORE any per-entry FS work). Closes the v1 path-traversal hole: a hand-edited bundle pointing at `../outside/file`, or a relative `--include` containing `..`, refuses before the verifier opens a single byte. Closed refusal reasons: `empty` / `absolute` / `backslash` / `dot_segment` / `dotdot_segment` / `empty_segment`. v1 constants: `BUNDLE_MAX_ENTRIES = 1024`, `BUNDLE_ENTRY_MAX_BYTES = 256 * 1024 * 1024` (256 MiB), `BUNDLE_LABEL_MAX = 128`, `BUNDLE_NOTES_MAX = 1024`. **Safety contract**: read-only on the state-store side (neither subcommand opens a contributor state-dir; the builder writes only `--out` atomically; the verifier writes nothing); bytes-and-presence only (no signature, no semantic JSON validation; `artifact_kind` is forensic context, not policy enforcement); fail-fast builder; collect-all verifier; determinism pinned by regression (same inputs + same `base_dir` + same `now_utc` + identical file contents → byte-identical bundle); auto-prune flag deliberately absent on both CLI surfaces, pinned by clap regression. **Path policy (locked)**: `relative_to_base_dir` ONLY in v1 — no enum, no `as_supplied` / `absolute` modes. Build-time resolution: absolute input → canonicalize, must start with canonical `base_dir`, strip prefix, record relative form (else `PathOutsideBaseDir`); relative input → recorded verbatim forward-slash-normalized, read from `<base_dir>/<input>`. Verify-time resolution: `effective_base_dir = base_dir_override.unwrap_or(bundle.base_dir)`, canonicalized; each entry resolves to `<effective_base_dir>/<entry.path>`. UTF-8 paths only (`NonUtf8Path` refuses non-UTF-8). **Symlinks under `--base-dir` are followed at hash time** via `std::fs::read` (which dereferences); the recorded `path` is the operator-supplied form, NOT the symlink target. **New CLI subcommand `omni-node operator contributor build-integrity-evidence-bundle`** with `--include <kind=path>` (required, repeatable, ≥1; split on the FIRST `=` so paths containing `=` are tolerated; unknown kind tags refuse at parse time), `--base-dir <path>` (required), `--out <path>` (required — atomic temp+rename), `--label <string>` (optional, ≤128 bytes UTF-8), `--notes <string>` (optional, ≤1024 bytes UTF-8), `--format events\|json\|pretty`. Closed-set events: `event=integrity_evidence_bundle_build_started base_dir=<...>`, one `event=integrity_evidence_bundle_entry_hashed kind=<...> path=<...> bytes=<...> blake3=<...>` per entry, `event=integrity_evidence_bundle_written path=<...> entry_count=<...>`; failure `event=integrity_evidence_bundle_build_failed reason=<closed-tag>` (tags: `empty_bundle` / `duplicate_entry` / `too_many_entries` / `bundle_label_too_long` / `notes_too_long` / `entry_too_large` / `entry_not_found` / `path_outside_base_dir` / `base_dir_invalid` / `non_utf8_path` / `io` / `malformed_json` / `invalid_include` / `unsupported_schema_version`). **New CLI subcommand `omni-node operator contributor verify-integrity-evidence-bundle`** with `--bundle <path>` (required), `--base-dir <path>` (optional override; defaults to `bundle.base_dir`), `--format events\|json\|pretty`. Closed-set events: `event=integrity_evidence_bundle_verify_started bundle=<path>[ base_dir_override=<path>]` fires FIRST (before any FS IO — operators always see a start event even when the bundle is missing / malformed / has an unsupported schema / fails envelope path validation; the `effective_base_dir` isn't known yet and lands on the summary line); per-entry `event=integrity_evidence_bundle_entry_ok` / `..._entry_size_mismatch` / `..._entry_hash_mismatch` / `..._entry_not_found` / `..._entry_read_error`; final `event=integrity_evidence_bundle_verify_summary effective_base_dir=<...> ok=<N> size_mismatch=<N> hash_mismatch=<N> not_found=<N> read_error=<N>`; envelope-level failure `event=integrity_evidence_bundle_verify_failed reason=<closed-tag>` + nonzero exit (bundle-read failures distinguish `reason=io` for missing/permission denied from `reason=malformed_json` for parse failures — single-sourced via the same closed-tag mapping as the build CLI so the two never get confused). Exit policy: 0 iff every entry is `Ok` AND no envelope-level error; any per-entry non-`Ok` → exit 1. **Out of scope (v1)**: bundle signature (Stage 12.20/12.21 already sign artifact bodies they care about — natural follow-on would be a sibling `SignedIntegrityEvidenceBundle` wrapper), semantic JSON validation (the verifier does NOT re-run Stage 12.20/12.21 signature verifiers on signed entries), `as_supplied` / `absolute` path policies, per-entry `label` (identity is `(kind, path)`), `restore_report` in the closed enum (no CLI emits a typed `RestoreReport` file today — operators capturing restore output use `other`), recursive directory bundling, automatic artifact discovery, expiry / `valid_through_utc`, multi-bundle composition. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.21 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 29 integrity-evidence-bundle integration tests cover the full accept/refuse matrix (build + verify happy path with 3 mixed kinds, **determinism — same inputs + `now_utc` produce byte-identical bundle**, sort order pinned by `(kind, path)`, `EmptyBundle` refusal, `DuplicateEntry` on `(kind, path)` collision, same-kind-different-paths OK, same-path-different-kinds OK in v1, `EntryTooLarge` at 256 MiB cap via sparse file with size check tripping before read, `TooManyEntries` at 1024 cap, `BundleLabelTooLong` / `NotesTooLong` refusals, `PathOutsideBaseDir` on absolute input outside `--base-dir`, absolute-input-under-base-dir recorded relative, verify all-`Ok` happy path, hash-mismatch collect-all proving other entries still walk, size-mismatch cheap pre-check with no hash burn, missing-entry collect-all `NotFound`, **Unix-only** chmod-000 `ReadError` collect-all, JSON round-trip, atomic writer round-trips via disk, wrapper schema v2 → `UnsupportedSchemaVersion`, `--base-dir` override relocates resolution root correctly, effective-base-dir-not-found → envelope-level refusal with no per-entry walk, **path traversal — relative `--include ../outside/file` refused with `dotdot_segment` BEFORE hashing**, **path traversal — relative input with backslash / `.` / empty segment refused with matching closed reason tag**, **path traversal — hand-edited bundle entry `../outside/file` refused with `InvalidRelativePath` at verify envelope BEFORE per-entry walk**, absolute-input-under-base-dir still succeeds after validator as defense-in-depth pin, **bundle read missing-file → `Io { NotFound }` (NOT `MalformedJson`) — pins the library variant the CLI's `reason=io` tag depends on**, **bundle read present-but-unparseable → `MalformedJson` (NOT `Io`) — counterpart pin**). 2 new CLI clap regressions pin both flag matrices: `build-integrity-evidence-bundle` (multi-`--include`, `--label`/`--notes`, format closed enum + unknown-format refusal, required-flag refusal sweep, deliberate rejection of `--no-prune-state-on-start`) and `verify-integrity-evidence-bundle` (required `--bundle`, optional `--base-dir` override, format closed enum + unknown-format refusal, deliberate rejection of `--no-prune-state-on-start`). New operator-runbook section walks the typical archival chain (build daily artifacts → assemble bundle → archive bundle JSON → operator verifies bytes via Stage 12.22 then signatures via Stage 12.20/12.21) + per-`EvidenceBundleError` triage. | `crates/omni-contributor/src/integrity_evidence_bundle.rs` (new — `build_integrity_evidence_bundle` / `verify_integrity_evidence_bundle` / `IntegrityEvidenceBundle` / `BundleEntry` / `BundleArtifactKind` / `BundleEntryOutcome` / `BundleVerifyReport` / `read_integrity_evidence_bundle_from_path` / `write_integrity_evidence_bundle_atomic` / `INTEGRITY_EVIDENCE_BUNDLE_SCHEMA_VERSION` + `BUNDLE_*` caps); extended `crates/omni-contributor/src/error.rs` (new `EvidenceBundleError`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `build-integrity-evidence-bundle` + `verify-integrity-evidence-bundle` subcommands + dispatch + run fns + 6 renderers + 2 clap regressions); `crates/omni-contributor/tests/integrity_evidence_bundle.rs` (new — 23 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.22 section), `docs/operator-runbook.md` (Stage 12.22 build + verify workflow section) | **Complete** |
| **Phase 5 Stage 12.21** — Signed integrity diffs. Local-only Ed25519-signed wrapper around a v1 `StateIntegrityDiffReport` plus a `sign-state-integrity-diff` CLI to produce one and a `verify-state-integrity-diff-signature` CLI to verify one — so operators can archive tamper-evident evidence of which baseline was compared against which current report, the exact diff that was produced, and which key signed the artifact. **No new protocol envelope. No canonical-byte changes on Stage 12.0–12.20 surfaces. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATE_INTEGRITY_DIFF_SCHEMA_VERSION` / `SIGNED_BASELINE_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** Stage 12.19's v1 `StateIntegrityDiffReport` is consumed UNCHANGED. New local-only schema `SIGNED_INTEGRITY_DIFF_SCHEMA_VERSION = 1`, independent of every existing schema constant — including `SIGNED_BASELINE_SCHEMA_VERSION`; v1 wrappers wrap v1 diffs only. New `omni_contributor::signed_diff` module with `pub fn sign_state_integrity_diff(diff, signer_pubkey_hex, signer_role, signed_at_utc, sign_fn) -> Result<SignedStateIntegrityDiff, SignedIntegrityDiffError>` (signer-agnostic — the caller supplies a closure that computes the signature; refuses non-v1 embedded diffs), `pub fn verify_signed_state_integrity_diff(wrapper, expected_signer_pubkey_hex) -> Result<(), SignedIntegrityDiffError>` (two-step refusal order: cheap `SignerPubkeyMismatch` pre-check first, then crypto verify; refuses non-v1 wrapper / non-v1 embedded diff at the same gate), `pub fn signed_integrity_diff_signing_input(...)` (builds `SIGNED_INTEGRITY_DIFF_DOMAIN \|\| bincode1::serialize(canonical_body)` mirroring the Stage 12.0–12.20 envelope pattern), `pub fn read_signed_integrity_diff_from_path` + `pub fn write_signed_integrity_diff_atomic` (operator-named atomic temp+rename writer). The four-role taxonomy is the Stage 12.20 `BaselineSignerRole { Operator, Contributor, Dispatcher, Coordinator }` enum **REUSED VERBATIM** — the variants are role names, not artifact-type names, so the same taxonomy applies to baselines and diffs alike. The `Baseline` prefix on the type name is a Stage 12.20 artifact; the public API name is preserved for back-compat. `SignedStateIntegrityDiff { schema_version, signed_at_utc, signer_pubkey_hex, signer_role, diff, signature_hex }` with `deny_unknown_fields` — embeds the full v1 `StateIntegrityDiffReport` so a captured wrapper is self-contained (no out-of-band diff file needed at verify time). New typed `SignedIntegrityDiffError { UnsupportedSchemaVersion, UnsupportedDiffSchemaVersion, Canonical, Signing, SignatureMismatch, SignerPubkeyMismatch, Io, MalformedJson }` — mirrors Stage 12.20 `SignedBaselineError` shape verbatim. New canonical-bytes constant `pub const SIGNED_INTEGRITY_DIFF_DOMAIN: &[u8] = b"OMNINODE-CONTRIBUTOR-SIGNED-INTEGRITY-DIFF:v1:"` — a NEW local-only domain separator, distinct from `SIGNED_BASELINE_DOMAIN` so the two trust artifacts can't be confused. **Safety contract**: trust is opt-in at the verifier (`--expected-signer-pubkey-hex` is the operator-supplied anchor — the wrapper's embedded pubkey is forensic context only); two-step refusal order means malicious wrappers with forged pubkeys never burn crypto cycles; canonical body covers every field of the wrapper EXCEPT `signature_hex`, so tampering `signed_at_utc` / `signer_role` / `signer_pubkey_hex` / any field of the embedded diff (counts, all three findings buckets, both inputs' provenance) all trip `SignatureMismatch`; **because the v1 diff already lifts both inputs' provenance (`baseline_generated_at_utc`, `current_generated_at_utc`, `baseline_state_dir`, `state_dir`, baseline/current state-versions and omni-contributor versions) into its own body, the signed wrapper transitively pins the baseline-vs-current context without needing a separate baseline-hash field**; determinism is pinned by regression (same diff + seed + `signed_at_utc` produces byte-identical wrappers); read-only — neither signing nor verifying opens a state-store, the signing CLI writes only the operator-named `--out` file (atomic temp+rename), and the verifier writes nothing. **New CLI subcommand `omni-node operator contributor sign-state-integrity-diff`** with `--diff-in <path>`, `--signer-seed <path>` (32-byte raw Ed25519 seed), `--signer-role operator\|contributor\|dispatcher\|coordinator`, `--out <path>`, `--format events\|json\|pretty`. Read-only on the state-store side; auto-prune flag deliberately absent (Stage 12.16/12.17/12.18/12.19/12.20 precedent), pinned by clap regression. **New CLI subcommand `omni-node operator contributor verify-state-integrity-diff-signature`** with `--signed-diff <path>`, `--expected-signer-pubkey-hex <hex>` (64-char lowercase Ed25519 pubkey trust anchor), `--format events\|json\|pretty`. Opens NO state-store; auto-prune flag deliberately absent, pinned by clap regression. Verifier JSON renderer emits a compact metadata view (`schema_version`, `signed_at_utc`, `signer_role`, `signer_pubkey_hex`, `signature_hex`, plus diff `schema_version` / `generated_at_utc` / `state_dir` / `baseline_state_dir` / `counts`) rather than re-printing the full embedded diff — verification's job is to attest authenticity, not to mirror bytes. Closed-set bare-stdout events: `event=signed_integrity_diff_signing_started`, `event=signed_integrity_diff_written`, `event=signed_integrity_diff_verify_started`, `event=signed_integrity_diff_verify_ok`, `event=signed_integrity_diff_verify_failed reason=<closed-tag>` (closed tags: `signer_pubkey_mismatch` / `signature_mismatch` / `unsupported_schema_version` / `unsupported_diff_schema_version` / `signing` / `canonical` / `io` / `malformed_json`). Any `SignedIntegrityDiffError` exits nonzero. **Out of scope (v1)**: signed-baseline-hash embedding in the wrapper (the v1 diff body already lifts both inputs' provenance, so adding a separate hash would be redundant), `state-integrity --baseline ... --sign-diff-out ...` one-shot (kept explicit so each artifact is auditable), printing the embedded diff JSON in the verifier (operators already have the wrapper on disk), `StateIntegrityDiffReport` schema change (embedded as v1; refuses non-v1), diff expiry / `valid_through_utc`, multi-signature / threshold signatures, diff encryption, revocation list, external signed-diff registry / publication. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.20 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 15 signed-diff integration tests cover the full accept/refuse matrix (sign + verify happy path, all four `BaselineSignerRole` variants round-trip, `SignerPubkeyMismatch` cheap pre-check before crypto, signature tamper → `SignatureMismatch`, embedded-diff tamper → `SignatureMismatch` proving the diff is in the canonical body, `signer_role` tamper → `SignatureMismatch`, `signed_at_utc` tamper → `SignatureMismatch`, wrapper schema v2 → `UnsupportedSchemaVersion`, embedded diff schema v2 → `UnsupportedDiffSchemaVersion`, sign refuses a non-v1 input diff, JSON round-trip preserves signature, **determinism — same diff + seed + `signed_at_utc` produces byte-identical wrapper**, canonical signing-input byte stability + domain prefix, empty-diff wrapper round-trips, `write_signed_integrity_diff_atomic` round-trips via disk). 2 new CLI clap regressions pin both flag matrices: `sign-state-integrity-diff` (all 4 roles parse, `--format` closed enum, required-flag refusal sweep, unknown-role refusal, unknown-format refusal, deliberate rejection of `--no-prune-state-on-start`) and `verify-state-integrity-diff-signature` (required `--signed-diff` and `--expected-signer-pubkey-hex`, `--format` closed enum, unknown-format refusal, deliberate rejection of `--no-prune-state-on-start`). New operator-runbook section walks signing → trust anchor capture → archival verification + composition with Stage 12.20 (signed baseline → diff → signed diff archival chain) + per-`SignedIntegrityDiffError` triage. | `crates/omni-contributor/src/signed_diff.rs` (new — `sign_state_integrity_diff` / `verify_signed_state_integrity_diff` / `SignedStateIntegrityDiff` / `signed_integrity_diff_signing_input` / `read_signed_integrity_diff_from_path` / `write_signed_integrity_diff_atomic` / `SIGNED_INTEGRITY_DIFF_SCHEMA_VERSION`); extended `crates/omni-contributor/src/canonical.rs` (new `SIGNED_INTEGRITY_DIFF_DOMAIN`), `crates/omni-contributor/src/error.rs` (new `SignedIntegrityDiffError`), `crates/omni-contributor/src/signed_baseline.rs` (doc-comment note on `BaselineSignerRole` reuse), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `sign-state-integrity-diff` + `verify-state-integrity-diff-signature` subcommands + dispatch + run fns + 6 renderers + 2 clap regressions); `crates/omni-contributor/tests/signed_integrity_diff.rs` (new — 15 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.21 section), `docs/operator-runbook.md` (Stage 12.21 sign + verify workflow section) | **Complete** |
| **Phase 5 Stage 12.20** — Signed integrity baselines. Local-only Ed25519-signed wrapper around a v1 `StateIntegrityReport` plus a `sign-state-integrity-baseline` CLI to produce one and `--signed-baseline` / `--baseline-pubkey-hex` flags on the Stage 12.19 diff surfaces to consume one — so operators can prove a baseline was produced by an expected key and detect hand-edited baselines before Stage 12.19 diffing. **No new protocol envelope. No canonical-byte changes on Stage 12.0–12.19 surfaces. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATE_INTEGRITY_DIFF_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** Stage 12.16's v1 `StateIntegrityReport` is consumed UNCHANGED. New local-only schema `SIGNED_BASELINE_SCHEMA_VERSION = 1`, independent of every existing schema constant; v1 wrappers wrap v1 reports only. New `omni_contributor::signed_baseline` module with `pub fn sign_state_integrity_baseline(report, signer_pubkey_hex, signer_role, signed_at_utc, sign_fn) -> Result<SignedStateIntegrityBaseline, SignedBaselineError>` (signer-agnostic — the caller supplies a closure that computes the signature; refuses non-v1 embedded reports), `pub fn verify_signed_state_integrity_baseline(baseline, expected_signer_pubkey_hex) -> Result<(), SignedBaselineError>` (two-step refusal order: cheap `SignerPubkeyMismatch` pre-check first, then crypto verify; refuses non-v1 wrapper / non-v1 embedded report at the same gate), `pub fn signed_baseline_signing_input(...)` (builds `SIGNED_BASELINE_DOMAIN \|\| bincode1::serialize(canonical_body)` mirroring the Stage 12.0–12.18 envelope pattern), `pub fn read_signed_baseline_from_path` + `pub fn write_signed_baseline_atomic` (operator-named atomic temp+rename writer). Closed `BaselineSignerRole { Operator, Contributor, Dispatcher, Coordinator }` with `as_str()` for stable wire tags and `snake_case` serde. `SignedStateIntegrityBaseline { schema_version, signed_at_utc, signer_pubkey_hex, signer_role, report, signature_hex }` with `deny_unknown_fields` — embeds the full v1 `StateIntegrityReport` so a captured wrapper is self-contained (no out-of-band report file needed at verify time). New typed `SignedBaselineError { UnsupportedSchemaVersion, UnsupportedReportSchemaVersion, Canonical, Signing, SignatureMismatch, SignerPubkeyMismatch, Io, MalformedJson }`. New canonical-bytes constant `pub const SIGNED_BASELINE_DOMAIN: &[u8] = b"OMNINODE-CONTRIBUTOR-SIGNED-INTEGRITY-BASELINE:v1:"` — a NEW local-only domain separator, not a change to any existing one. **Safety contract**: trust is opt-in at the verifier (`--baseline-pubkey-hex` is the operator-supplied anchor — the wrapper's embedded pubkey is forensic context only); two-step refusal order means malicious wrappers with forged pubkeys never burn crypto cycles; canonical body covers every field of the wrapper EXCEPT `signature_hex`, so tampering `signed_at_utc` / `signer_role` / `signer_pubkey_hex` / any field of the embedded report all trip `SignatureMismatch`; determinism is pinned by regression (same report + seed + `signed_at_utc` produces byte-identical wrappers); read-only — neither signing nor verifying opens a state-store, and the signing CLI writes only the operator-named `--out` file (atomic temp+rename). **New CLI subcommand `omni-node operator contributor sign-state-integrity-baseline`** with `--baseline-in <path>`, `--signer-seed <path>` (32-byte raw Ed25519 seed), `--signer-role operator\|contributor\|dispatcher\|coordinator`, `--out <path>`, `--format events\|json\|pretty`. Read-only on the state-store side; auto-prune flag deliberately absent (Stage 12.16/12.17/12.18/12.19 precedent), pinned by clap regression. **Additive `--signed-baseline <path>` + `--baseline-pubkey-hex <hex>` flags** on the existing `state-integrity --baseline ...` and `state-integrity-diff` subcommands: clap-level mutually exclusive with `--baseline`; clap requires `--baseline-pubkey-hex` whenever `--signed-baseline` is set; `state-integrity-diff` clap-enforces required-unless-the-other on the baseline pair. Runtime backstop on BOTH subcommands: when `--baseline-pubkey-hex` is supplied alongside `--baseline` (without `--signed-baseline`), the CLI emits `event=warn context=baseline_pubkey_hex_unused` via the shared `resolve_diff_baseline` helper. Clap accepts the combination on both subcommands (the `conflicts_with` / `required_unless_present` interactions short-circuit the `requires` check); the runtime warning is the uniform backstop so the trust-anchor-was-dropped state is observable to log scrapers. Solo `--baseline-pubkey-hex` (with no `--baseline` and no `--signed-baseline`) is still rejected at clap-parse time on both subcommands. Closed-set bare-stdout events: `event=signed_baseline_signing_started`, `event=signed_baseline_written`, `event=signed_baseline_verified`, `event=signed_baseline_refused reason=<closed-tag>` (closed tags: `signer_pubkey_mismatch` / `signature_mismatch` / `unsupported_schema_version` / `unsupported_report_schema_version` / `signing_decode_error` / `canonical_encoding_error` / `other`). Any `SignedBaselineError` exits nonzero. **Out of scope (v1)**: current-report signing (v1 signs baselines only), `StateIntegrityReport` schema change (embedded as v1; refuses non-v1), `StateIntegrityDiffReport` signature (trust chain ends at the baseline), baseline expiry / `valid_through_utc`, multi-signature / threshold signatures, baseline encryption, revocation list, external signed-baseline registry / publication. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.19 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 16 signed-baseline integration tests cover the full accept/refuse matrix (sign + verify happy path, all four `BaselineSignerRole` variants round-trip, `SignerPubkeyMismatch` cheap pre-check before crypto, signature tamper → `SignatureMismatch`, embedded-report tamper → `SignatureMismatch` proving the report is in the canonical body, `signer_role` tamper → `SignatureMismatch`, `signed_at_utc` tamper → `SignatureMismatch`, wrapper schema v2 → `UnsupportedSchemaVersion`, embedded report schema v2 → `UnsupportedReportSchemaVersion`, sign refuses a non-v1 input report, JSON round-trip preserves signature, **determinism — same report + seed + `signed_at_utc` produces byte-identical wrapper**, canonical signing-input byte stability + domain prefix, Stage 12.19 composition — signed wrapper → verify → feed embedded report into `diff_state_integrity_reports`, empty-report wrapper round-trips, `write_signed_baseline_atomic` round-trips via disk). 1 new CLI clap regression pins the `sign-state-integrity-baseline` flag matrix (all 4 roles parse, `--format` closed enum, required-flag refusal sweep, unknown-role refusal, deliberate rejection of `--no-prune-state-on-start`). Existing `state_integrity_flag_parse_smoke` and `state_integrity_diff_flag_parse_smoke` extended for the new `--signed-baseline` / `--baseline-pubkey-hex` mutual-exclusion + requires-pair coverage. New operator-runbook section walks signing → trust anchor capture → CI gate consumption (`state-integrity --signed-baseline --baseline-pubkey-hex --fail-on-new-error`) + forensic two-JSON form + per-`SignedBaselineError` triage. | `crates/omni-contributor/src/signed_baseline.rs` (new — `sign_state_integrity_baseline` / `verify_signed_state_integrity_baseline` / `SignedStateIntegrityBaseline` / `BaselineSignerRole` / `signed_baseline_signing_input` / `read_signed_baseline_from_path` / `write_signed_baseline_atomic` / `SIGNED_BASELINE_SCHEMA_VERSION`); extended `crates/omni-contributor/src/canonical.rs` (new `SIGNED_BASELINE_DOMAIN`), `crates/omni-contributor/src/error.rs` (new `SignedBaselineError`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `sign-state-integrity-baseline` subcommand + dispatch + run fn + 3 renderers + clap regression; additive `--signed-baseline` / `--baseline-pubkey-hex` on `state-integrity` and `state-integrity-diff` + extended clap regressions; new shared `resolve_diff_baseline` helper); `crates/omni-contributor/tests/signed_baseline.rs` (new — 16 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.20 section), `docs/operator-runbook.md` (Stage 12.20 sign + verify workflow section) | **Complete** |
| **Phase 5 Stage 12.19** — Local integrity-report diff. Read-only `diff_state_integrity_reports` helper + two CLI surfaces (`state-integrity-diff` for two-JSON forensic comparison and `state-integrity --baseline` for live-vs-baseline CI gating) that classify findings between two `StateIntegrityReport` snapshots as `new`, `resolved`, or `unchanged`. **No new protocol envelope. No canonical-byte changes. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** Stage 12.16's v1 `StateIntegrityReport` is consumed UNCHANGED. `omni_contributor::integrity` (extended; no new module): new `pub const STATE_INTEGRITY_DIFF_SCHEMA_VERSION: u32 = 1` (independent of `STATE_INTEGRITY_REPORT_SCHEMA_VERSION`), `DiffOptions<'a> { now_utc, require_state_dir_match }`, `DiffCounts { new, resolved, unchanged, new_ok, new_warn, new_error, resolved_ok, resolved_warn, resolved_error }`, `StateIntegrityDiffReport { schema_version, generated_at_utc, baseline_generated_at_utc, current_generated_at_utc, baseline_omni_contributor_version, current_omni_contributor_version, baseline_state_version, current_state_version, state_dir, baseline_state_dir, counts, new_findings, resolved_findings, unchanged_findings }`, `pub fn diff_state_integrity_reports(baseline, current, opts) -> Result<StateIntegrityDiffReport, IntegrityDiffError>`, and (review fix) `pub fn diff_presentation_view<'a>(diff, summary_only) -> Cow<'a, StateIntegrityDiffReport>` — the single redaction helper the CLI's events / pretty / JSON stdout renderers AND the `--json-out` mirror all route through so `--summary-only` is honored uniformly across every output path (clones and clears `unchanged_findings` when set; preserves `counts.unchanged` verbatim). All three finding vectors sorted by the same `(session_id, kind, path, reason_tag)` key the scanner uses → diff JSON is byte-stable on identical inputs. **Identity key** is the bit-exact tuple `(kind, session_id, path, reason_tag)` — the same composite Stage 12.16 already sorts by. **Metadata drift refusal**: two findings sharing an identity tuple but disagreeing on `severity` or `recommended_action` refuse with typed `IntegrityDiffError::FindingMetadataDrift` carrying both sides for forensics. v1 has no scanner path that produces such drift (closed-set Stage 12.16 emission is deterministic), so the refusal is a "shouldn't happen" guard for tampered or non-Stage-12.16 reports. **No `drifted_findings` vector in v1** — future stages can promote if needed. New typed `IntegrityDiffError { UnsupportedBaselineSchemaVersion, UnsupportedCurrentSchemaVersion, IncompatibleStateVersion, StateDirMismatch, FindingMetadataDrift, MalformedBaseline, MalformedCurrent, Io }`. **Safety contract**: pure read-only — the library function consumes two `&StateIntegrityReport` references and never opens a state-store; `state-integrity-diff` opens no state-store at all (pure JSON-to-JSON); `state-integrity --baseline` runs the existing Stage 12.16 scanner first then the diff helper, preserving the unconditional `auto_prune = false` posture (no `--no-prune-state-on-start` flag exposed; pinned by clap regression). Both inputs must have `schema_version == STATE_INTEGRITY_REPORT_SCHEMA_VERSION`. **Strict `state_version` equality**. **`state_dir` drift is informational by default** (recorded in `state_dir` + `baseline_state_dir`); `--require-state-dir-match` makes it a refusal for host-pinned baselines (default OFF since CI baselines are commonly captured on a different host). **`omni_contributor_version` drift is informational only** — recorded in `baseline_omni_contributor_version` + `current_omni_contributor_version`, never a refusal. `generated_at_utc` is always ignored for finding equality but recorded in provenance. **Two new CLI surfaces backed by the same helper**: new `omni-node operator contributor state-integrity-diff` subcommand with required `--baseline <path>` + required `--current <path>`, `--require-state-dir-match`, `--fail-on-new`, `--fail-on-new-error`, `--summary-only`, `--format events|json|pretty`, optional `--json-out <path>`; AND additive `--baseline <path>` / `--fail-on-new` / `--fail-on-new-error` / `--summary-only` / `--require-state-dir-match` flags on the existing `state-integrity` subcommand (when `--baseline` is set, the command switches to rendering the diff report instead of the raw scan; existing `--format` / `--json-out` / `--fail-on-warn` all compose via OR — any one tripping → exit 1). Closed-set bare-stdout events: `event=state_integrity_diff_started`, `event=integrity_diff_new`, `event=integrity_diff_resolved`, `event=integrity_diff_unchanged` (suppressed by `--summary-only`), `event=state_integrity_diff_summary` (with the full `DiffCounts` shape), `event=state_integrity_diff_json_written`. Any `IntegrityDiffError` exits nonzero. **Exit-code policy**: default exit 0 (informational); `--fail-on-new` exits 1 when `counts.new > 0`; `--fail-on-new-error` exits 1 when `counts.new_error > 0`; both can be set with OR semantics. `--fail-on-new-error` is the recommended CI mode (warnings stay green). **Out of scope (v1)**: `StateIntegrityReport` v1 schema change (consumed as-is), `drifted_findings` vector, three-way diff, fuzzy / semantic match, cross-state-version diff, baseline signing or hash-pinning, watcher / daemon mode, automatic baseline refresh. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.18 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 17 integrity-diff integration tests cover the full accept/refuse matrix (identical reports → empty diff + full `unchanged`; empty baseline → all `new`; empty current → all `resolved`; mixed `new` + `resolved` + `unchanged`; diff byte-stability across re-runs; baseline schema v2 → `UnsupportedBaselineSchemaVersion`; current schema v2 → `UnsupportedCurrentSchemaVersion`; state-version mismatch → `IncompatibleStateVersion`; state_dir mismatch passes by default + refuses with flag; omni_contributor_version drift informational; severity drift → `FindingMetadataDrift`; recommended-action drift → `FindingMetadataDrift`; JSON round-trip preserves report; per-severity sub-counts; live scan against baseline JSON round-trips end-to-end via the same `serde_json::to_vec_pretty` path the CLI's `--json-out` uses; `SessionIntegritySummary` drift doesn't affect finding diff; **review-fix `--summary-only` redaction regression — `diff_presentation_view` elides `unchanged_findings` from both JSON stdout and `--json-out` while preserving `counts.unchanged`**). 1 new CLI clap regression pins the `state-integrity-diff` flag matrix (required `--baseline` and `--current`, all toggles, `--format` closed enum, unknown-`--format` rejection, deliberate rejection of `--no-prune-state-on-start`); the existing `state_integrity_flag_parse_smoke` is extended to verify the Stage 12.19 `--baseline` / `--fail-on-new` / `--fail-on-new-error` / `--summary-only` / `--require-state-dir-match` flags parse on `state-integrity`. New operator-runbook section walks the CI gate workflow (one-time baseline capture → per-PR `--baseline --fail-on-new-error` invocation) + forensic two-JSON comparison + per-`IntegrityDiffError` triage. | `crates/omni-contributor/src/integrity.rs` (extended — `diff_state_integrity_reports` / `StateIntegrityDiffReport` / `DiffOptions` / `DiffCounts` / `STATE_INTEGRITY_DIFF_SCHEMA_VERSION`); extended `crates/omni-contributor/src/error.rs` (new `IntegrityDiffError`), `crates/omni-contributor/src/lib.rs` (re-exports); `crates/omni-node/src/contributor_cli.rs` (new `state-integrity-diff` subcommand + dispatch + run fn + 3 renderers + clap regression; additive `--baseline` mode on existing `state-integrity` + extended clap regression); `crates/omni-contributor/tests/state_integrity_diff.rs` (new — 16 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.19 section), `docs/operator-runbook.md` (Stage 12.19 diff workflow section) | **Complete** |
| **Phase 5 Stage 12.18** — Local cleanup-quarantine restore. First-class rollback for the Stage 12.17 cleanup applier — a `restore-state-cleanup-quarantine` command that consumes the v1 `quarantine-manifest.json` Stage 12.17 wrote, BLAKE3-verifies every quarantined body, path-checks every destination, restores bytes back into the contributor state-dir, and (by default) re-marks the matching `seen/<ns>/<key>` markers. **No new protocol envelope. No canonical-byte changes. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION` / `CLEANUP_PLAN_SCHEMA_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** Stage 12.17's v1 `QuarantineManifest` is consumed UNCHANGED — its `source_relative` + `quarantine_relative` + `blake3_hex` + `source_finding_kind` fields are sufficient for both body restore (via `store.write_archived_bytes`) and seen-marker restore (via `seen_for_verified` → `parse_seen_relative` → `store.mark_seen`). `omni_contributor::cleanup` (extended; no new module): `pub fn verify_quarantine_manifest(source) -> Result<QuarantineManifest, QuarantineRestoreError>` (stand-alone parse + schema-version + state-version + plan_id-pinning helper) and `pub fn restore_state_cleanup_quarantine(store, opts) -> Result<QuarantineRestoreReport, QuarantineRestoreError>` (the full restorer). Closed `QuarantineRestoreSource<'a> { PlanDir(&Path), QuarantineRoot { quarantine_dir, plan_id } }`. `QuarantineRestoreOptions<'a> { source, dry_run, verify_only, overwrite_existing, restore_seen_markers, allow_restore_orphan_assignments, now_utc }` — `verify_only` wins when both flags are set (Stage 12.15 precedent verbatim). `QuarantineRestoreReport { plan_id, mode, manifest_schema_version, source_state_version, files_restored, seen_markers_restored, bytes_restored, quarantine_dir, outcomes }`; per-entry `QuarantineRestoreOutcome { entry_index, source_relative, source_finding_kind, status, seen_marker_written }` with closed `status` = `"applied"` / `"would_apply"` / `"verify_only"`. New typed `QuarantineRestoreError { QuarantineDirNotFound, ManifestMissing, MalformedManifest, UnsupportedManifestVersion, IncompatibleSourceStateVersion, PlanIdMismatch, UnsafeRelativePath, ManifestFileMissing, BlakeMismatch, DestinationExists, SeenMarkerPathBlocked, GatedRestoreRequired, UnknownFindingKind, State, Io }`. **Safety contract — five explicit phases mirroring Stage 12.15 plus the Stage 12.18 review-fix split of Phase C**: **Phase A** parses the manifest, refuses on schema/state/plan_id mismatch, path-checks every entry against the closed `verified/sessions/<64hex>/...` whitelist (refusal BEFORE any IO with typed `UnsafeRelativePath`), classifies every `source_finding_kind` against the closed Stage 12.17 set (refusal with `UnknownFindingKind` for any forward-incompatible tag), and refuses `orphan_replacement_assignments` entries without `--allow-restore-orphan-assignments` (typed `GatedRestoreRequired`). **Phase B** runs whenever `verify_only || !dry_run`: reads every `<quarantine-dir>/<plan_id>/<quarantine_relative>` and BLAKE3-verifies against the manifest's `blake3_hex` (fail-fast `BlakeMismatch`); real-restore keeps the bytes in memory so Phase C3 writes without re-reading. **Phase C1** is the body destination preflight (all-or-nothing `DestinationExists` refusal unless `--overwrite-existing`). **Phase C2** is the Stage 12.18 review-fix seen-marker destination preflight — when `restore_seen_markers` is on (default), every entry's would-be `seen/<ns>/<key>` destination is checked and any non-file occupant refuses the whole restore with `SeenMarkerPathBlocked` BEFORE any body is written; `--overwrite-existing` does NOT cover marker hazards. **Phase C3** runs the writes: `store.write_archived_bytes` per body + (when `restore_seen_markers`) `store.mark_seen` per marker. **Stage 12.17 quarantine subtree is left intact** — operator manages retention manually. **Closed mode matrix** (`verify_only` > `dry_run` > `restore`) mirrors Stage 12.15. **Auto-prune forced OFF unconditionally** — the `--no-prune-state-on-start` flag is deliberately absent (Stage 12.16/12.17 precedent), pinned by clap regression. New CLI subcommand `omni-node operator contributor restore-state-cleanup-quarantine` with `--contributor-state-dir`, mutually-exclusive `--quarantine-plan-dir` xor (`--quarantine-dir + --plan-id`) [clap enforces both the conflict and the pair], `--dry-run`, `--verify-only`, `--overwrite-existing`, `--no-restore-seen-markers`, `--allow-restore-orphan-assignments`, `--format events|json|pretty`. Closed-set bare-stdout events: `event=state_store_opened`, `event=restore_quarantine_started`, `event=verify_only_quarantine_file`, `event=would_restore_quarantine_file`, `event=restore_quarantine_file`, `event=restore_quarantine_complete`. Any `QuarantineRestoreError` exits nonzero. **Out of scope (v1)**: Tier-A action restoration (Stage 12.17 Tier-A had no quarantine payload — documented gap; operator re-runs the watcher), `--purge-stray` entry restoration (operator opted to purge at cleanup time), cross-state-dir restore (strict `source_state_version == STATE_VERSION` equality), manifest schema v2 (v1 is sufficient), drift check against post-cleanup state (not recorded; operator runs `state-integrity` post-restore). **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.17 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 16 cleanup-quarantine-restore integration tests cover the full accept/refuse matrix (`verify_quarantine_manifest` parses a v1 manifest, end-to-end cleanup → restore for a tampered join, `--verify-only` on intact quarantine writes nothing, `--dry-run` writes nothing + emits `would_apply`, `--verify-only` wins over `--dry-run`, corrupted quarantine byte → `BlakeMismatch` refusal, path-traversal in manifest → `UnsafeRelativePath` BEFORE IO, schema-version mismatch → `UnsupportedManifestVersion`, plan_id mismatch (renamed dir) → `PlanIdMismatch`, pre-existing destination → `DestinationExists` with state-dir untouched, `--overwrite-existing` replaces destination bytes, `--no-restore-seen-markers` restores body only, unknown `source_finding_kind` → `UnknownFindingKind`, `orphan_replacement_assignments` entry requires `--allow-restore-orphan-assignments` gate flag, **hash-tight rollback regression — post-restore `source_integrity_hash` exactly equals pre-cleanup `source_integrity_hash` when markers are restored and no Tier-A actions were in the plan**, **review-fix marker preflight regression — a directory planted at the marker path refuses the whole restore with `SeenMarkerPathBlocked` BEFORE any body write, and `--no-restore-seen-markers` escapes the preflight**). 1 CLI clap regression pins the `--quarantine-plan-dir` xor `--quarantine-dir + --plan-id` mutual-exclusion + pair-requires, the `--dry-run`/`--verify-only`/`--overwrite-existing`/`--no-restore-seen-markers`/`--allow-restore-orphan-assignments` toggles, the `--format` closed enum, and the deliberate rejection of `--no-prune-state-on-start`. New operator-runbook section walks the inspect → verify-only → restore → re-scan loop plus per-`QuarantineRestoreError` triage + the rollback hash invariant. | `crates/omni-contributor/src/cleanup.rs` (extended — `restore_state_cleanup_quarantine` / `verify_quarantine_manifest` / `QuarantineRestoreSource` / `QuarantineRestoreOptions` / `QuarantineRestoreReport` / `QuarantineRestoreOutcome` / `check_quarantine_relative_path` / `classify_finding_kind`); extended `crates/omni-contributor/src/error.rs` (new `QuarantineRestoreError`), `crates/omni-contributor/src/lib.rs` (re-exports); `crates/omni-node/src/contributor_cli.rs` (new `restore-state-cleanup-quarantine` subcommand + dispatch + run fn + 3 renderers + clap regression); `crates/omni-contributor/tests/state_cleanup_quarantine_restore.rs` (new — 15 integration tests); `docs/stage12-contributor-protocol.md` (Stage 12.18 section), `docs/operator-runbook.md` (Stage 12.18 restore workflow section) | **Complete** |
| **Phase 5 Stage 12.17** — Local state-dir cleanup planner / applier. Two-step `plan-state-cleanup` + `apply-state-cleanup` flow that composes Stage 12.16 integrity findings + the Stage 12.13 audit projection into a deterministic JSON plan of closed-set cleanup actions, then walks the plan one action at a time with apply-time drift detection and a Stage 12.14-shaped quarantine. **No new protocol envelope. No canonical-byte changes. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** New `omni_contributor::cleanup` module with frozen `CLEANUP_PLAN_SCHEMA_VERSION = 1` and `QUARANTINE_MANIFEST_SCHEMA_VERSION = 1`. Closed `CleanupActionKind` enum (9 variants split into **tier A** = reversible seen-marker edits `RemoveSeenMarker` / `WriteSeenMarker` / `RemoveSeenFile`, **tier B** = archive-before-delete `QuarantineVerifiedFile` / `QuarantineAndUnmarkJoin` / `QuarantineAndUnmarkAssignment` / `QuarantineAndUnmarkSupersession`, **tier B gated** = `QuarantineAndUnmarkPartial` requiring `--allow-invalid-partial-cleanup` + `QuarantineAndUnmarkOrphanAssignment` requiring `--allow-orphan-assignments`). `StateCleanupPlan { schema_version, plan_id, created_at_utc, state_dir, source_integrity_hash, omni_contributor_version, actions, cleanup_plan_hash }` where `plan_id` = first 16 hex chars of `BLAKE3(state_dir ‖ source_integrity_hash ‖ created_at_utc)`, `source_integrity_hash` = BLAKE3 of the Stage 12.16 report with `generated_at_utc` + `state_dir` blanked, `cleanup_plan_hash` mirrors Stage 12.11's `repair_plan_hash` recipe (BLAKE3 over canonical JSON with the field cleared). `pub fn plan_state_cleanup(report, audit_orphans, &PlanOptions) -> Result<StateCleanupPlan, CleanupError>` (pure projection; read-only). `pub fn apply_state_cleanup(store, &StateCleanupPlan, &ApplyOptions) -> Result<CleanupReport, CleanupError>` — re-runs `scan_state_integrity_with_audit_orphans` for drift detection BEFORE any mutation; per gated session, re-runs `compute_audit_health` and refuses on orphan-id-set drift (typed `OrphanAuditDrift`); per Tier-B action, copies source bytes to `<quarantine-dir>/<plan_id>/<source_relative>` + BLAKE3-verifies + removes source + unmarks seen; writes `quarantine-manifest.json` LAST so partial applies leave it visibly missing (Stage 12.14 invariant verbatim). New typed `CleanupError { Integrity, Status, State, SourceIntegrityDrift, PlanHashMismatch, GateRequired, OrphanAuditDrift, QuarantineCollision, MalformedPlan, UnsupportedPlanVersion, UnsafePlanPath, Io }`. Two new narrow state-store primitives: `ContributorStateStore::remove_verified_relative(source_relative)` (path-whitelisted via existing Stage 12.14 validator, idempotent NotFound-is-benign) and `ContributorStateStore::unmark_seen(ns, key)` (counterpart of `mark_seen`, idempotent). **Stage 12.17 scanner extensions (additive, no report-schema bump)**: new `pub fn scan_state_integrity_with_audit_orphans(store, opts) -> Result<(StateIntegrityReport, HashMap<String, Vec<String>>), IntegrityError>` returns the v1 report unchanged + a sibling per-session orphan-assignment id map; `FindingKind::StraySeenFile` (declared but never emitted in Stage 12.16) is now actively emitted by a new `scan_stray_seen_files` walker that catches files directly under `seen/`, files under unknown namespace dirs, and shape-malformed keys (non-64-hex or missing `<sid>--` for prefixed namespaces); the reverse `scan_seen_marker_consistency` walk is now shape-strict so the same file never accumulates both a `StaleSeenMarker` and a `StraySeenFile` finding. **Safety contract**: plan is read-only; apply re-runs Stage 12.16's scan first (typed `SourceIntegrityDrift` refusal on mismatch); plan-hash recompute (typed `PlanHashMismatch`); gated actions refused at apply time without their flag (typed `GateRequired`, refusal happens BEFORE any FS mutation); pre-existing `<quarantine-dir>/<plan_id>/` refused (typed `QuarantineCollision`); **every action `path` + `seen_marker_path` validated against the per-kind whitelist before any IO (typed `UnsafePlanPath`)** — `cleanup_plan_hash` self-consistency does NOT vouch for path safety since hand-edited plans can recompute the hash trivially; **apply runs three explicit phases — Phase A quarantines the bytes + BLAKE3-verifies the copy, Phase B writes the quarantine manifest atomically (tempfile + rename), Phase C removes sources and unmarks seen markers** — so any Phase A or B failure leaves the state-dir byte-identical to pre-apply. Both CLIs open the state-store with `auto_prune = false` unconditionally — neither command exposes `--no-prune-state-on-start` (Stage 12.16 review-fix precedent), pinned by clap regression. **Two new CLI subcommands**: `omni-node operator contributor plan-state-cleanup` with `--contributor-state-dir`, optional `--session-id`, optional `--integrity-json <path>` (consume a pre-baked Stage 12.16 report; drift between supplied report and live scan warned at plan time and refused at apply time), required `--out <path>` (atomic temp+rename), `--format events|json|pretty`; and `omni-node operator contributor apply-state-cleanup` with `--contributor-state-dir`, required `--plan <path>`, required `--quarantine-dir <path>` (consistent layout even for tier-A-only plans), `--dry-run`, `--allow-invalid-partial-cleanup`, `--allow-orphan-assignments`, `--purge-stray` (skip quarantine for `QuarantineVerifiedFile` only), `--format events|json|pretty`. Closed-set bare-stdout events: `event=cleanup_plan_written`, `event=cleanup_action_planned`, `event=cleanup_plan_built`, `event=cleanup_started`, `event=cleanup_action_applied`, `event=would_apply_action`, `event=cleanup_action_skipped`, `event=cleanup_complete`. Any `CleanupError` exits nonzero. **No automatic chain into Stage 12.11 reassign** — operators chain `apply-state-cleanup && plan-session-reassign && apply-session-reassign` themselves. **No InvalidSession / InvalidAggregate cleanup in v1** — those route through Stage 12.14 `archive-session --move` or manual triage. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.16 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 19 cleanup integration tests cover the full accept/refuse matrix (clean state → empty plan, plan-hash self-consistency, tier-A round-trips for stale + missing seen markers, tier-B round-trip for tampered join with quarantine + unmark + post-apply re-scan clean, stray verified file round-trip, source-integrity-drift refusal, plan-hash mismatch refusal, gate refusal + accept for `--allow-invalid-partial-cleanup`, dry-run state + quarantine untouched, pre-existing `<plan_id>/` collision refusal, plan ⇄ JSON round-trip preserves hashes, `InvalidSession` produces no v1 action, cleanup quarantine doesn't collide with Stage 12.14 archive layout, **path-traversal refusal in `RemoveSeenFile` / `RemoveSeenMarker` / tier-B `path` / `seen_marker_path` (review fix — every malicious plan is refused with `UnsafePlanPath` even when `cleanup_plan_hash` is recomputed to be self-consistent), quarantine-write failure leaves state-dir byte-identical (review fix — Phase A→B→C ordering proves no source removal can happen before the quarantine manifest is durable)**). 4 new Stage 12.16 scanner regression tests pin StraySeenFile emission (under `seen/` root, under unknown namespace, shape-malformed key with no double-emit of StaleSeenMarker) + clean-state orphan side-channel. 2 new CLI clap regressions pin the `plan-state-cleanup` / `apply-state-cleanup` flag matrices + the deliberate rejection of `--no-prune-state-on-start` on both. New operator-runbook section walks the plan → dry-run → apply → re-scan loop plus per-`CleanupError` triage. | `crates/omni-contributor/src/cleanup.rs` (new — `plan_state_cleanup` / `apply_state_cleanup` / `StateCleanupPlan` / `CleanupAction` / `CleanupActionKind` / `CleanupReport` / `CleanupActionOutcome` / `PlanOptions` / `ApplyOptions` / `QuarantineManifest` / `QuarantineEntry` / `source_integrity_hash_hex` / `cleanup_plan_hash_hex` / `CLEANUP_PLAN_SCHEMA_VERSION` / `QUARANTINE_MANIFEST_SCHEMA_VERSION`); extended `crates/omni-contributor/src/integrity.rs` (new `scan_state_integrity_with_audit_orphans` + `scan_stray_seen_files` walker + shape-strict reverse-walk), `crates/omni-contributor/src/state.rs` (new `remove_verified_relative` + `unmark_seen`), `crates/omni-contributor/src/error.rs` (new `CleanupError`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `plan-state-cleanup` + `apply-state-cleanup` subcommands + dispatch + run fns + 6 renderers + 2 clap regressions); `crates/omni-contributor/tests/state_cleanup_plan.rs` (new), `crates/omni-contributor/tests/state_integrity_scan.rs` (extended); `docs/stage12-contributor-protocol.md` (Stage 12.17 section), `docs/operator-runbook.md` (Stage 12.17 cleanup workflow section) | **Complete** |
| **Phase 5 Stage 12.16** — Local state-dir integrity scan + repair suggestions. Adds a read-only diagnostics surface so operators can detect drift, half-finished repairs, stale seen markers, stray files, and audit-projection regressions in their contributor state-dir without touching disk. **No new protocol envelope. No canonical-byte changes. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` / `ARCHIVE_MANIFEST_SCHEMA_VERSION` / `STATUS_SCHEMA_VERSION` / `NET_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** New `omni_contributor::integrity` module: `scan_state_integrity(store, &ScanOptions) -> Result<StateIntegrityReport, IntegrityError>` composes the Stage 12.7 parse-only loaders, the Stage 12.3 / 12.11 verifier functions, the Stage 12.13 `load_verified_restart_snapshot` + `compute_audit_health` projection, and (optionally) the Stage 12.15 `restore_session_archive(verify_only=true, dry_run=true)` walk. Closed `FindingSeverity { Ok, Warn, Error }`; closed `FindingKind` covering every detectable structural anomaly (`InvalidSession`, `InvalidJoin`, `InvalidAssignment`, `InvalidPartial`, `InvalidSupersession`, `InvalidAggregate`, `StaleSeenMarker`, `MissingSeenMarker`, `StrayVerifiedFile`, `StraySeenFile`, `OrphanReplacementAssignments`, `PartialApplySupersession`, `ReassignTriagable`, `NotReassignTriagable`, `ArchiveManifestMalformed`, `ArchiveBlakeMismatch`, `ArchiveCoveredSession`); closed-set static-string `RecommendedAction` labels (`"run session-status"`, `"run plan-session-reassign --reason invalid-partial"`, `"run plan-session-reassign --reason missing-partial"`, `"run archive-session --require-status aggregated"`, `"run archive-session --require-status expired-incomplete"`, `"run archive-session --verify-only"`, `"run restore-session-archive --verify-only"`, `"delete stale seen marker"`, `"clean state-dir orphan replacements before retry"`, `"operator triage required"`, `"none"`) the scanner emits but **never executes**. `StateIntegrityReport { schema_version=1, generated_at_utc, state_dir, state_version, omni_contributor_version, sessions_scanned, sessions_verified, counts_ok, counts_warn, counts_error, sessions: Vec<SessionIntegritySummary>, findings: Vec<IntegrityFinding> }` with frozen `STATE_INTEGRITY_REPORT_SCHEMA_VERSION = 1`. Findings sorted deterministically by `(session_id, kind, path, reason_tag)` so repeated scans on the same disk state produce byte-identical JSON. New typed `IntegrityError { State, Status, Io }` — scanner-aborting only (per-artifact problems become findings, not variants). **Safety contract**: zero writes — no `write_verified_json`, no `mark_seen`, no `cascade_remove_session`, no archive writes; the scanner uses parse-only loaders + re-invokes verifier functions on parsed bodies in memory. `--session-id` filter restricts every session-scoped finding (Stage 12.13 rejection notes, per-session deep scans, per-session seen-marker walks, and per-session archive walks) to one session, while cross-session structural findings still surface. New CLI subcommand `omni-node operator contributor state-integrity` with `--contributor-state-dir`, optional `--session-id`, optional `--include-archives <path>`, `--format events|json|pretty`, optional `--json-out <path>`, `--fail-on-warn`. Auto-prune is forced OFF unconditionally on this subcommand — the `--no-prune-state-on-start` flag other subcommands expose is deliberately rejected here, because the Stage 12.7 expiry cascade would silently delete the exact "expired/incomplete session" subtrees an integrity scan is supposed to surface. Exit-code policy: default = 1 when `counts_error > 0`; `--fail-on-warn` = 1 when `counts_warn + counts_error > 0`. JSON-mode keeps stdout report-only (operational chatter goes to stderr) so a `jq` pipeline works directly; events / pretty keep their bare-stdout posture. Closed-set bare-stdout events: `event=state_store_opened`, `event=session_integrity_summary`, `event=integrity_finding kind=… severity=… session_id=… path=… reason_tag=… recommended_action="…"`, `event=state_integrity_summary sessions_scanned=… sessions_verified=… counts_ok=… counts_warn=… counts_error=…`. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.15 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 18 integrity-scan integration tests cover the full accept/refuse matrix (clean state with scanner-writes-nothing guard, tampered session/join/assignment/partial/aggregate bodies → matching `Invalid*` findings, stale seen marker → `StaleSeenMarker`, missing seen marker → `MissingSeenMarker`, stray file → `StrayVerifiedFile`, `--session-id` filter restricts session-scoped findings, clean `--include-archives` walk → `ArchiveCoveredSession` Ok, corrupt archive → `ArchiveBlakeMismatch` error, missing manifest → `ArchiveManifestMalformed` error, JSON round-trip preserves report, deterministic ordering across re-runs, empty state-dir clean scan, **clean supersession seen marker round-trips without false `StaleSeenMarker`** (review fix — reverse-walk maps `seen/assignment-supersessions/<sid>--<id>` to the correct `verified/sessions/<sid>/supersessions/<id>.json` path), **scanner leaves expired session subtree on disk** (review fix — CLI opens with `auto_prune = false`)); 1 CLI clap regression pins the `--session-id` / `--include-archives` / `--format events\|json\|pretty` / `--json-out` / `--fail-on-warn` matrix + unknown-`--format` rejection + the deliberate rejection of `--no-prune-state-on-start` (review fix). New operator-runbook section walks the routine health check → per-session drill → pre-restore archive walk → CI strict-mode workflows plus the per-`recommended_action` triage table. Stage 12.13 audit-projection findings (`OrphanReplacementAssignments`, `PartialApplySupersession`, `ReassignTriagable`, `NotReassignTriagable`) are lifted into the scanner so audit drift is now visible to integrity-scan consumers in addition to `session-status --format events\|pretty`. | `crates/omni-contributor/src/integrity.rs` (new — `scan_state_integrity` / `StateIntegrityReport` / `IntegrityFinding` / `SessionIntegritySummary` / `FindingKind` / `FindingSeverity` / `RecommendedAction` / `ScanOptions` / `STATE_INTEGRITY_REPORT_SCHEMA_VERSION`); extended `crates/omni-contributor/src/error.rs` (new `IntegrityError`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `state-integrity` subcommand + dispatch + run fn + 3 renderers + clap regression test); `crates/omni-contributor/tests/state_integrity_scan.rs` (new); `docs/stage12-contributor-protocol.md` (Stage 12.16 section), `docs/operator-runbook.md` (Stage 12.16 integrity scan workflow section) | **Complete** |
| **Phase 5 Stage 12.15** — Local session archive restore / import validation. Inverse of Stage 12.14: reads `<archive-session-dir>/manifest.json`, validates every file's BLAKE3 + safe-path + state-dir version compatibility, then writes each archived byte back into the operator's state-dir at the same relative path. **No new protocol envelope. No canonical-byte changes. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` bump. No `ARCHIVE_MANIFEST_SCHEMA_VERSION` bump. No new gossipsub topic. No SNIP wire change.** Restore is **byte-identical replay** of the bytes Stage 12.14 captured — every Stage 12.0–12.11 verifier (`verify_execution_session` / `verify_contributor_join` / `verify_work_assignment` / `verify_assignment_supersession`) accepts the restored bodies unchanged because their signatures are over canonical bytes that survived the copy. New `omni_contributor::restore` module: `restore_session_archive(store, opts) -> RestoreReport`, `verify_archive_manifest(source) -> ArchiveManifest`, `RestoreSource { SessionDir, ArchiveRoot { archive_dir, session_id } }`, `RestoreOptions { source, dry_run, verify_only, overwrite_existing, include_results, now_utc }`. New typed `RestoreError { ArchiveNotFound, ManifestMissing, MalformedManifest, UnsupportedManifestVersion, SessionIdMismatch, IncompatibleSourceStateVersion, UnsafeRelativePath, DisallowedRelativePath, ManifestFileMissing, BlakeMismatch, DestinationExists, Io, State }`. New state-store primitive `ContributorStateStore::write_archived_bytes(source_relative, bytes, overwrite_existing)`: enforces relative-path safety (no `..`, no absolute, no backslash), prefix whitelist matching the Stage 12.14 archive layout (`verified/sessions/<64-hex>/...`, `seen/sessions/<64-hex>`, `seen/aggregates/<64-hex>`, `seen/{joins,assignments,partials,peer-adverts,assignment-supersessions}/<64-hex>--*`, `results/result-links/<64-hex>.link.json`), destination-existence refusal (unless `overwrite_existing`), and atomic tempfile+rename via the existing Stage 12.7 helper. Path violations surface as new typed `StateError::UnsafeRelativePath` / `StateError::DisallowedRelativePath` / `StateError::DestinationExists` variants. **Safety contract** (closed mode matrix): `--dry-run` parses manifest + per-entry path safety without reading archive bytes or writing destinations; `--verify-only` adds full BLAKE3 walk over every archived file but no destination writes; real restore adds destination-existence preflight (all-or-nothing — if any destination exists and `--overwrite-existing` is false, refuse BEFORE writing anything) + writes every byte verbatim. BLAKE3 mismatch is fail-fast (no retry). `source_state_version` is **strict equality only** (`== STATE_VERSION`) in v1; future stages can add a migration story. Session-id binding defends against renamed archive directories. New CLI subcommand `omni-node operator contributor restore-session-archive` with `--contributor-state-dir`, mutually-exclusive `--archive-session-dir` xor (`--archive-dir + --session-id`), `--dry-run` + `--verify-only` (NOT mutually exclusive; `--verify-only` wins when both passed), `--overwrite-existing` (default off), `--include-results` (default off — skips `results/result-links/...` entries even if archive contains them), `--no-prune-state-on-start`. Closed-set bare-stdout events: `event=restore_started` (carries the manifest's `session_overall_status` + `audit_coherence` as informational closed-set fields), `event=restore_file`, `event=verify_only_file`, `event=would_restore_file`, `event=restore_skipped_result_link reason=include_results_off`, `event=restore_complete`. Any `RestoreError` exits nonzero. **No automatic `session-status` chain after restore** — operators chain `&& session-status` themselves. Operator runbook documents the "stop the watcher before restoring" caveat (Stage 12.7's state-dir is operator-local). **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.14 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 18 restore integration tests cover the full accept/refuse matrix (dry-run / verify-only / round-trip / post-archive-move recreate / restart preload accepts restored session / BLAKE3 mismatch / missing archive file / path traversal / absolute path / disallowed path / existing-destination preflight all-or-nothing / overwrite-existing replaces / result-link skipped vs restored under `--include-results` / incompatible state-version / unsupported manifest version / session-id mismatch / `verify_archive_manifest` helper); 6 state-store regressions pin `write_archived_bytes` safety (path traversal, absolute, backslash, disallowed prefix, every whitelisted prefix accepted, existing-destination refused unless overwrite); 1 CLI clap regression pins the `--archive-session-dir` xor `--archive-dir + --session-id` mutual exclusion, the `--dry-run` + `--verify-only` independence, and the `--overwrite-existing` / `--include-results` toggles. | `crates/omni-contributor/src/restore.rs` (new — `restore_session_archive` / `verify_archive_manifest` / `RestoreSource` / `RestoreOptions` / `RestoreReport`); extended `crates/omni-contributor/src/state.rs` (new public `write_archived_bytes` + private `validate_archive_relative_path` + `is_blake3_hex`), `crates/omni-contributor/src/error.rs` (new `RestoreError` + `StateError::{UnsafeRelativePath,DisallowedRelativePath,DestinationExists}`), `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (new `restore-session-archive` subcommand + dispatch + run fn + clap regression); `crates/omni-contributor/tests/restore_session_archive.rs` (new), `crates/omni-contributor/tests/state_store_unit.rs` (extended); `docs/stage12-contributor-protocol.md` (Stage 12.15 section), `docs/operator-runbook.md` (Stage 12.15 restore workflow section) | **Complete** |
| **Phase 5 Stage 12.14** — Local session archival + state-dir compaction. Adds operator lifecycle management for completed / aggregated / expired session subtrees. **No new protocol envelope. No canonical-byte changes. No `schema_version` bump on any in-tree envelope. No `STATE_VERSION` bump. No new gossipsub topic. No SNIP wire change.** New `omni_contributor::archive` module: `archive_session(store, opts)` walks `verified/sessions/<session_id>/...` plus session-keyed `seen/*` markers, copies each file byte-for-byte to `<archive-dir>/<session_id>/...`, BLAKE3-verifies every destination, and writes a manifest LAST so a partial copy is visibly missing the manifest without source corruption. `ArchiveManifest { schema_version=1, session_id, generated_at_utc, source_state_version, omni_contributor_version, session_overall_status, audit_coherence, mode, require_status, include_results, files: Vec<ArchivedFile> }`; `ArchivedFile { source_relative, archive_relative, blake3_hex, bytes }`. Closed `ArchiveMode { Copy, Move }` and `ArchiveStatusRequirement { Any, Complete, Aggregated, CompletePartials, ExpiredIncomplete }`. New typed `ArchiveError { SessionNotPresent, StatusRequirementUnmet, ArchiveAlreadyExists, BlakeMismatch, Io, Status, State }`. **Safety contract**: `--dry-run` returns a manifest without touching the FS; `--copy` (default) leaves the source untouched; `--move` runs `ContributorStateStore::cascade_remove_session` ONLY after every BLAKE3 verifies AND the manifest write succeeds — a partial-copy failure never deletes source; BLAKE3 mismatch is fail-fast (no retry). Stage 12.7's private `cascade_remove_session` free fn is lifted to a public `ContributorStateStore::cascade_remove_session(&self, session_id)` method so expiry-driven pruning and operator-driven `--move` archival converge on one cascade implementation. New CLI subcommand `omni-node operator contributor archive-session` with `--contributor-state-dir`, `--session-id`, `--archive-dir`, `--require-status any|complete|aggregated|complete-partials|expired-incomplete` (default `complete` = `Aggregated`-or-`CompletePartials`), `--copy` xor `--move` (default `--copy`), `--include-results` (default off — copies only `results/result-links/<posted_id>.link.json` when present; `results/contributor-results/<job_id>.json` is left alone because the mapping is per-job, not per-session), `--dry-run`. Closed-set bare-stdout events: `event=archive_started`, `event=archive_file`, `event=archive_complete`, `event=would_archive_file`, `event=would_archive_complete`. Any `ArchiveError` exits nonzero so automation detects refusals. Stage 12.13's `compute_audit_health` `recommended_action` closed set grows from 5 → 7 strings: `Coherent + Aggregated` recommends `"run archive-session --require-status aggregated"`; `Coherent + ExpiredIncomplete` recommends `"run archive-session --require-status expired-incomplete"`. **`CompletePartials` is deliberately NOT recommended for archival** — operator may still want the aggregate to land. JSON renderer is unchanged (v3 `SessionStatusReport` JSON schema frozen at Stage 12.12; audit lives in `events` + `pretty` renderers only). **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No omni-store / omni-net wire change.** No Stage 12.0–12.13 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 13 archive integration tests cover the full accept/refuse matrix (dry-run no-op, copy + verify BLAKE3, copy preserves source, move removes source + seen markers, status-requirement refusal on `InProgress`, `Any` accepts `InvalidState`, missing-session refusal, existing-archive-dir refusal, `--include-results` copies only `result-links/`, sibling sessions untouched, manifest serde round-trip, `ExpiredIncomplete` accepted under matching requirement, Stage 12.11 supersession files + seen markers archived); 1 state-store regression pins the public cascade matches the prune-cascade behavior; 3 audit regressions pin archive recommendations for `Aggregated` + `ExpiredIncomplete` and explicit non-recommendation for `CompletePartials`; 1 CLI clap regression pins the `--require-status` closed enum, the `--copy` xor `--move` mutual exclusion, and the `--include-results` / `--dry-run` toggles. New operator-runbook section walks the dry-run → copy → inspect → optional move workflow plus recovery from partial copy / BLAKE3 mismatch / manual restore. | `crates/omni-contributor/src/archive.rs` (new — `archive_session` / `ArchiveManifest` / `ArchivedFile` / `ArchiveMode` / `ArchiveOptions` / `ArchiveStatusRequirement` / `ARCHIVE_MANIFEST_SCHEMA_VERSION`); extended `crates/omni-contributor/src/state.rs` (new public `cascade_remove_session` method delegating to the existing private free fn), `crates/omni-contributor/src/error.rs` (new `ArchiveError`), `crates/omni-contributor/src/lib.rs` (module + re-exports), `crates/omni-contributor/src/resume.rs` (audit closed-set extension); `crates/omni-node/src/contributor_cli.rs` (new `archive-session` subcommand + dispatch + run fn + clap regression test); `crates/omni-contributor/tests/archive_session_integration.rs` (new), `crates/omni-contributor/tests/state_store_unit.rs` (extended), `crates/omni-contributor/tests/phase_b_partial_apply.rs` (extended); `docs/stage12-contributor-protocol.md` (Stage 12.14 section), `docs/operator-runbook.md` (Stage 12.14 archival workflow section) | **Complete** |
| **Phase 5 Stage 12.13** — Supersession-aware resume/watch hardening + audit ergonomics. Closes three runtime gaps in Stage 12.11/12.12 consumers — restart preload completeness, out-of-order supersession arrival, apply-session-reassign Phase B partial-apply windows — and adds an operator-facing audit ergonomics layer over the existing v3 status report. **No new envelope, no new canonical bytes, no schema_version bump, no new gossipsub topic.** New `omni_contributor::resume` module: `load_verified_restart_snapshot(store)` walks the state-dir in dependency order (sessions → joins → assignments → supersessions), re-running `verify_execution_session` / `verify_contributor_join` / `verify_work_assignment` / `verify_assignment_supersession` on the way and returning typed `RestartSnapshot { sessions, joins_by_session, assignments_by_session, supersessions_by_session }` + `RestartReport { *_accepted, *_rejected, rejection_notes }`. Rejection notes carry the stable `SessionVerifyOutcome::reason_tag` from Stage 12.12 so log scrapers can pattern-match. Aggregate re-verify is deliberately skipped at preload — the first `session-status` build re-runs `verify_aggregated_result_with_supersessions`. `run_watch_sessions` consumes the snapshot and warms `sessions` + `assignments_by_session` + `supersessions_by_session` caches up front (partials and aggregates remain lazy); one new `event=state_store_restart_loaded` line at startup + one `event=warn context=state_store_restart_load_rejected …` per rejection. `handle_assignment_supersession` is reworked into a two-pass strategy: first pass calls `process_assignment_supersession_announcement(ann, snip, Some(session), None)` to verify announcer-sig + body-schema + body-sig + drift + session-pinning and return the parsed body; if the in-memory assignments cache covers every referenced id, the handler runs `verify_assignment_supersession` directly against the slice for full reference resolution; otherwise it accepts on the first-pass checks alone and emits `event=supersession_partial_verify session_id=… supersession_id=… unresolved_assignment_count=K` so the operator sees the looser path. The aggregate verifier at `session-status` time re-checks references against the full state-dir snapshot. Forged body signatures are still rejected — best-effort does NOT bypass body-sig. `apply-session-reassign` Phase B is split into B1..B4: B1 writes every replacement to the state-dir + marks seen, B2 writes the supersession + marks seen, B3 mesh-broadcasts each replacement (failures warn but state-dir is already coherent), B4 mesh-broadcasts the supersession ONLY when every B3 broadcast succeeded (preserves the "replacements observable on mesh before supersession" invariant via warn-and-skip). The trailing `event=reassign_applied` line gains closed-set `supersession_broadcast=<broadcast_ok\|broadcast_failed\|skipped_replacement_broadcast_failed\|skipped_no_publish>` and `replacement_broadcast_failures=N` fields. New audit ergonomics: `compute_audit_health(report) -> AuditHealth { coherence: AuditCoherence, triagable_by_reassign: bool, recommended_action: &'static str }` — closed `AuditCoherence` enum (`Coherent` / `PartialApplySupersession { supersession_id, unresolved_count }` / `OrphanReplacementAssignments { assignment_ids }` / `NotReassignTriagable` / `ReassignTriagable`) + closed-set `recommended_action` strings (`"none"`, `"run plan-session-reassign --reason missing-partial"`, `"run plan-session-reassign --reason invalid-partial"`, `"clean state-dir orphan replacements before retry"`, `"operator triage required"`). Pure projection over the v3 report — no I/O, no verifier re-run. `session-status --format events` adds one `event=audit_health` line per report; `--format pretty` adds an "Audit health" section under "Notes:"; `--format json` is **unchanged** (v3 schema frozen at Stage 12.12 — audit is renderer-only). **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store / omni-net wire change.** No Stage 12.0–12.12 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 3 state-resume regressions (preload caches assignments, restart preserves status, restart drops forged supersession with reason_tag note); 6 Phase B / audit projection tests (orphan replacement, partial-apply supersession, coherent happy path, reassign-triagable vs not-triagable on InvalidState, source_status_hash drift on Phase B1-only state); 4 out-of-order supersession handler tests (best-effort accept with missing reference, full verification with complete slice, no-session pinning, forged body-sig still rejected). New operator-runbook section walks the supersession / reassign workflow + Phase B partial-apply recovery + `invalid_artifacts` reading. | `crates/omni-contributor/src/resume.rs` (new — `RestartSnapshot` / `RestartReport` / `load_verified_restart_snapshot` / `AuditCoherence` / `AuditHealth` / `compute_audit_health`); extended `crates/omni-contributor/src/lib.rs` (module + re-exports); `crates/omni-node/src/contributor_cli.rs` (watch-sessions preload via snapshot loader, supersession handler two-pass, Phase B reorder, audit renderers); `crates/omni-contributor/tests/state_resume_integration.rs` (Stage 12.13 restart tests), `crates/omni-contributor/tests/phase_b_partial_apply.rs` (new), `crates/omni-contributor/tests/watch_session_supersession_out_of_order.rs` (new); `docs/stage12-contributor-protocol.md` (Stage 12.13 section), `docs/operator-runbook.md` (Phase 5 contributor protocol supersession/reassign workflow section) | **Complete** |
| **Phase 5 Stage 12.12** — Invalid-partial triage for supersession. Builds on Stage 12.9 status reporter + Stage 12.11 supersession verifier + Stage 12.11 reassign CLI. Enables `apply-session-reassign --reason invalid-partial` to triage an `InvalidState` session, **only** when every chain-link failure is a tampered partial whose retiring assignment is named in the plan. Stage 12.9 `STATUS_SCHEMA_VERSION` 2 → 3: adds `invalid_artifacts: Vec<InvalidArtifactStatus>` to `SessionStatusReport` (closed externally-tagged enum — `invalid_session` / `invalid_join` / `invalid_assignment` / `invalid_partial` / `invalid_supersession` / `invalid_aggregate`; each variant carries `reason_tag: String`). Free-form `notes` stays populated alongside so existing operator dashboards keep working — but automation reads `invalid_artifacts` only. Every site that currently flips `any_chain_invalid` also pushes a structured entry; the invariant `invalid_artifacts.is_empty() <=> overall_status != InvalidState` is mechanically pinned. New dedicated `SessionVerifyOutcome::reason_tag(&self) -> &'static str` method returns stable tags — NOT derived from `format!("{outcome:?}")` — pinned variant-by-variant by `session_verify_outcome_reason_tags_are_stable`. Orphan partials surface as `InvalidPartial { reason_tag: "unmatched" }` (no separate `OrphanPartial` variant). New `RepairError::InvalidStateNotTriagable { kind: &'static str, context: String }` with closed `kind` tags (`invalid_session` / `invalid_join` / `invalid_assignment` / `invalid_supersession` / `invalid_aggregate` / `invalid_partial_not_in_plan` / `invalid_state_without_diagnostics`). New `check_reassign_eligible_allowing_invalid_partials(status, plan)`: only path that accepts `InvalidState`, and only when every `invalid_artifacts` entry is `InvalidPartial` whose `assignment_id` is in the plan's superseded set; every other status arm forwards to `check_repair_eligible` (preserving the Stage 12.10/12.11 contract that `NoSession` / `NoAssignments` / `CompletePartials` / `Aggregated` / `ExpiredIncomplete` keep their typed errors). New companion `check_invalid_partial_plan_eligible(status)` is the **plan-time** mirror of the same policy expressed against the candidate target set the planner is about to emit — pins the planner contract that `emits a plan <=> apply will accept it` so a status with `InvalidPartial(A) + InvalidJoin(...)` is refused at plan time with the same `InvalidStateNotTriagable { kind, context }` surface, not at apply time after the operator has already paid plan-creation cost. `build_session_repair_plan_with_reason` branches on `(strategy, reason)`: only `ReassignMissing + InvalidPartial` calls the plan-time precheck (not `check_repair_eligible`) and selects actions from `invalid_artifacts`; all other reason / strategy combos keep the existing `partial_present == false && !superseded` path. `apply-session-reassign` dispatches on `plan.actions[0].reason` — `InvalidPartial` calls the new helper, every other reason calls `check_repair_eligible`. **Apply-time mixed-reason defense**: a hand-edited plan with mixed `SupersessionReason` values is refused before any eligibility / drift / SNIP / mesh / state-dir work — the planner emits a uniform reason per plan but the plan is unsigned, so the apply path enforces it. `MissingPartial` / `OperatorRebalance` reassigns + every `ReannounceMissing` reannounce continue hard-refusing `InvalidState`. Renderers: `events` adds `event=invalid_artifact session_id=... kind=... reason_tag=... [assignment_id=... \| contributor_pubkey_hex=... \| supersession_id=...]`; `pretty` adds an "Invalid artifacts" section under counts; `json` unchanged (serde already exposes the field). **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No A–F flow. SNIP only. No new gossipsub topic. No omni-store wire change.** No Stage 12.0–12.11 canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 7 status-reporter emission tests cover the full diagnostic matrix (forged partial / forged join / tampered assignment / tampered aggregate / early-return invalid session / orphan partial / invariant); 4 planner action-selection tests cover the `--reason invalid-partial` branch (selection on InvalidState, superseded-skip, `MissingPartial` / `OperatorRebalance` / `ReannounceMissing` still-refuse-InvalidState); 10 integration tests cover the helper's full accept/refuse matrix (accept when all InvalidPartial covered; refuse on extra InvalidJoin / InvalidAssignment / InvalidAggregate / InvalidSupersession / InvalidSession / InvalidPartial-not-in-plan / `ReannounceMissing` strategy / empty-diagnostics defense; forward to `check_repair_eligible` for non-InvalidState arms); 1 reason-tag stability test pins every existing variant tag. | `crates/omni-contributor/src/status.rs` (schema bump + InvalidArtifactStatus + structured emissions at every existing `any_chain_invalid` site), `crates/omni-contributor/src/session_verify.rs` (new `reason_tag` method), `crates/omni-contributor/src/repair.rs` (new helper + planner branch), `crates/omni-contributor/src/error.rs` (new typed variant), `crates/omni-contributor/src/lib.rs` (re-exports), `crates/omni-node/src/contributor_cli.rs` (apply-time dispatch + mixed-reason defense + renderer additions), `crates/omni-contributor/tests/invalid_partial_triage.rs` (new), `crates/omni-contributor/tests/{status_report.rs,repair_plan.rs}` (extended), `docs/stage12-contributor-protocol.md` (Stage 12.12 section) | **Complete** |
| **Phase 5 Stage 12.11** — Signed assignment supersession. Builds on Stage 12.3 verifier helpers + Stage 12.7 state-dir + Stage 12.9 status reporter + Stage 12.10 repair planner. New coordinator-signed off-chain envelope `WorkAssignmentSupersession` v1 (`session_id`, `supersession_id` = BLAKE3 over canonical signing body, sorted-unique `superseded_assignment_ids`, **non-empty** sorted-unique disjoint `replacement_assignment_ids`, closed `SupersessionReason` enum (`MissingPartial` / `InvalidPartial` / `OperatorRebalance` / `Custom { label }`), `created_at_utc`, `coordinator_pubkey_hex`, `coordinator_signature_hex`). v1 is **replacement-only** — empty replacement list yields `SchemaError::SupersessionEmptyReplacement` (review-mandated; abandonment requires a separate aggregate-cancellation envelope which is explicitly out of scope). New `verify_assignment_supersession` (schema + session-binding + coord-binding + ID-derivation + sig + reference-resolution legs) plus `verify_aggregated_result_with_supersessions` aggregate-verifier extension that unions every supersession's superseded IDs out of the partial-coverage requirement (sequential chains supported; classic `verify_aggregated_result` is now `verify_aggregated_result_with_supersessions(..., &[])` bit-for-bit unchanged on zero supersessions). New gossipsub topic `TOPIC_CONTRIBUTOR_SESSION_ASSIGNMENT_SUPERSESSION` (`omni/contributor/session/assignment-supersession/v1`) carrying pointer-only `NetworkWorkAssignmentSupersessionAnnouncement`. Stage 12.7 `STATE_VERSION` 1 → 2 (adds `verified/sessions/<id>/supersessions/<id>.json` and `seen/assignment-supersessions/`). Stage 12.9 `STATUS_SCHEMA_VERSION` 1 → 2 (adds `active_assignment_count`, `superseded_assignment_count`, `supersession_count`, `supersessions: Vec<SupersessionStatus>`, per-assignment `superseded` + `superseded_by_supersession_id`; `missing_assignment_ids` becomes active-only; status reporter loads supersessions BEFORE partials so a tampered partial under `InvalidPartial` supersession does not flip overall status to `InvalidState`). Stage 12.10 `REPAIR_PLAN_SCHEMA_VERSION` 1 → 2 (adds `RepairStrategy::ReassignMissing` and `RepairAction::ReassignAssignment`; the `ReannounceMissing` planner now filters out any active-missing assignment already named in a verified supersession). Two new CLI subcommands under `omni-node operator contributor`: `plan-session-reassign` (mutually-exclusive `--build-status` / `--status-report`, closed-enum `--reason missing-partial|invalid-partial|operator-rebalance` default `missing-partial`, optional `--coordinator-pubkey-hex` hint) and `apply-session-reassign` (recomputes `source_status_hash` against the v2 projection — now binds per-assignment `superseded` + `superseded_by_supersession_id` so a supersession arriving between plan and apply trips drift; refuses non-`ReassignMissing` strategy / mixed action kinds / `InvalidState` per v1 spec; re-fetches + re-verifies session via `--session-snip-root`; pins coordinator seed to `session.coordinator_pubkey_hex`; builds + signs one replacement `WorkAssignment` per `ReassignAssignment` action AND a single covering `WorkAssignmentSupersession`; `--dry-run` validates without writes; **Phase A — SNIP-publishes every body up-front** (replacements then supersession) WITHOUT any state-dir mutation or mesh broadcast, so a transient SNIP failure leaves the source projection unchanged and the operation is safely retryable; **Phase B — once every SNIP body is durable**, dual-writes to the state-dir + broadcasts `NetworkWorkAssignedAnnouncement` per replacement unless `--no-publish-announcements` + marks seen, then dual-writes the supersession + broadcasts `NetworkWorkAssignmentSupersessionAnnouncement` unless `--no-publish-announcements` + marks seen; publish ordering matters so watchers never see a supersession naming assignment IDs they have not yet stored). `watch-sessions` polls the new topic via `poll_assignment_supersessions`; `handle_assignment_supersession` upgrades to full reference resolution when both the in-memory session cache and the state-dir's verified-assignment slice are available, otherwise accepts on announcer-sig + body-schema + body-sig + drift (aggregate verifier re-checks references at aggregate time). `session-status` events + pretty renderers gain top-line `active=` / `superseded=` / `supersessions=` counts, per-assignment `superseded=yes\|no superseded_by=<id\|->`, and new `event=supersession_status` / supersessions-table outputs; JSON unchanged (serde already exposes the new fields). **Reuses Stage 12.10's `source_status_hash` + apply-time eligibility re-check. New gossipsub topic; new state-dir layout; new status-report fields; new repair-plan strategy + action — all behind schema-version bumps so consumers can detect upgrade.** **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation logic. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No omni-store wire change. No A–F flow. SNIP-only.** No Stage 12.0–12.10 canonical-byte changes (Stage 12.10 `verify_aggregated_result` re-checked bit-for-bit on the zero-supersession path). Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. Supersession negatives test suite covers schema (incl. v1 empty-replacement refusal + disjointness), session/coord binding, ID-derivation, sig, and reference-resolution rejection arms; aggregate-supersession integration covers happy path + sequential chains + post-supersession-missing-partial; status-reporter regression pins `tampered_partial_under_invalid_partial_supersession_is_not_invalid_state`; planner regression pins `ReannounceMissing` filter-against-in-flight-supersession; four `process_assignment_supersession_announcement` unit tests pin happy-path (with + without session context), `session_id` drift, and body-coord-pubkey drift under session pinning; clap regression pins `--build-status` xor `--status-report`, `--reason` parse, inverted `--no-publish-announcements`, and `--dry-run` toggle on both new subcommands; omni-net topic test re-asserts the full topic-constant inventory (count 13 → 14). | `crates/omni-contributor/src/supersession.rs` (new), `crates/omni-contributor/src/supersession_verify.rs` (new); extended `crates/omni-contributor/src/{canonical.rs,net.rs,relay.rs,session_verify.rs,session.rs,state.rs,status.rs,repair.rs,error.rs,lib.rs}` (schema bumps + new processor + state-dir layout + status v2 fields + repair v2 strategy/action + module wiring), `crates/omni-net/src/topics.rs` (new topic constant), `crates/omni-node/src/contributor_cli.rs` (two new subcommands + watch-sessions handler + renderer updates + clap regression test), `crates/omni-contributor/tests/{supersession_byte_stability.rs,supersession_negatives.rs,aggregate_supersession_integration.rs,session_announcement_processing.rs,status_report.rs,repair_plan.rs}` (new + extended), `docs/stage12-contributor-protocol.md` (Stage 12.11 section) | **Complete** |
| **Phase 5 Stage 12.10** — Local pooled-session repair planner. Builds on Stage 12.9 status reports + Stage 12.7 state-dir loaders + Stage 12.3 verifier helpers. New `omni_contributor::repair` module: `SessionRepairPlan` v1 (local-only, **unsigned**, schema-versioned), `RepairStrategy` closed enum (`ReannounceMissing` v1; `ReassignMissing` deferred to Stage 12.11+ pending supersession design), `RepairAction` closed enum (`ReannounceAssignment { assignment_id, stage_index, contributor_pubkey_hex }` v1 only), `build_session_repair_plan` library entry, `repair_plan_hash_hex` BLAKE3 integrity tag, `source_status_hash_hex` projection over `(session_id, sorted [(assignment_id, partial_present)] pairs)`. Halt-finding-driven scope: `verify_aggregated_result` requires every assignment in the supplied slice to appear in `aggregate.partial_refs` exactly once, so adding a replacement assignment for a missing partial would trip `AggregateMissingPartialFor` at aggregate time — `ReassignMissing` cannot ship without a Stage 12.3 supersession envelope, so it's deferred. Test suite includes `aggregate_verifier_rejects_extra_assignment_without_partial` as a pinning fixture for the Stage 12.11+ design discussion. Two new CLI subcommands under `omni-node operator contributor`: `plan-session-repair` (read-only; either `--build-status` builds the Stage 12.9 report on the fly OR `--status-report <path>` loads a pre-built report, mutually exclusive via clap; emits one `ReannounceAssignment` action per missing valid partial when status is `InProgress`; refuses `NoSession` / `NoAssignments` / `CompletePartials` / `Aggregated` / `InvalidState` / `ExpiredIncomplete` with typed errors; no `--allow-invalid-state` flag) and `apply-session-repair` (recomputes `repair_plan_hash` + recomputes `source_status_hash` from current state-dir + refuses on either drift; re-fetches + re-verifies session via `--session-snip-root`; re-verifies each referenced assignment via `verify_work_assignment` before any publish; `--dry-run` validates without SNIP/mesh; non-dry-run republishes each assignment's JSON bytes verbatim to SNIP — content-addressed → same root, `assignment_id` + coordinator signature + SNIP root all byte-preserved — and broadcasts a fresh `NetworkWorkAssignedAnnouncement` unless `--no-publish-announcements`). **Reuses existing `WorkAssignment` + `NetworkWorkAssignedAnnouncement` envelopes; no new gossipsub topic (reuses `TOPIC_SESSION_WORK_ASSIGNED`); no state-dir mutation on apply.** **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing / reputation logic. No exclusive claim / lease model. No proof mode / on-chain verification / chain proof allowlist. No omni-net topic change. No omni-store wire change.** No Stage 12.0–12.9 schema or canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 14 unit + integration tests cover the full refusal matrix (NoSession / NoAssignments / CompletePartials / Aggregated / InvalidState / ExpiredIncomplete), the accepted `InProgress` path with one action per missing partial, deterministic ordering under input shuffle, `repair_plan_hash` serde round-trip, `source_status_hash` ignoring noise (`generated_at_utc`, `notes`) while changing when `partial_present` flips, the end-to-end state-dir → status → plan flow, drift detection when a partial arrives between plan and apply, and the verifier-coverage-rejection pinning fixture. 1 new omni-node clap regression test pinning the `--build-status` xor `--status-report` mutual exclusion, the inverted `--no-publish-announcements` posture, and the `--dry-run` toggle. | `crates/omni-contributor/src/repair.rs` (new); extended `crates/omni-contributor/src/error.rs` (new `RepairError` variant), `crates/omni-contributor/src/lib.rs` (module + re-exports), `crates/omni-node/src/contributor_cli.rs` (2 new subcommands + clap regression test), `crates/omni-contributor/tests/repair_plan.rs` (new), `docs/stage12-contributor-protocol.md` (Stage 12.10 section) | **Complete** |
| **Phase 5 Stage 12.9** — Local pooled-session progress monitor. Read-only observability over a Stage 12.7 `--contributor-state-dir`. New `omni_contributor::status` module: `SessionStatusReport` v1 (local-only, **unsigned**, schema-versioned), `AssignmentStatus`, `SessionOverallStatus` closed enum (`NoSession` / `NoAssignments` / `InProgress` / `CompletePartials` / `Aggregated` / `ExpiredIncomplete` / `InvalidState`), `build_session_status_report` library entry. Loads session + joins + assignments + partials + peer adverts + optional aggregate from the state-dir; re-runs `verify_execution_session` + `verify_contributor_join` + `verify_work_assignment` + `verify_partial_result` + `verify_peer_advertisement_body` + full-chain `verify_aggregated_result` before counting anything as valid. Counts are valid-only; tampered chain links surface as per-artifact `notes` plus `InvalidState` overall status, never as inflated counts. Failing peer adverts produce notes but never flip the overall status — adverts are routing helpers, not chain links. Duplicate partials are filesystem-impossible (state layout keys partials by `assignment_id`) so `duplicate_partial_assignment_ids` is always empty in v1, kept for forward compatibility. New CLI subcommand `omni-node operator contributor session-status` with `--format events|json|pretty`, optional `--json-out` mirror (best-effort `std::fs::write`; dashboard snapshot, not a protocol artifact), `--fail-on-incomplete` exit-code gate (success only on `CompletePartials` or `Aggregated`), `--include-expired` for advert counting, and Stage 12.7 `--no-prune-state-on-start`. Reporter performs no SNIP fetch, no mesh broadcast, no chain interaction, and no mutation beyond the optional JSON mirror. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing logic. No proof mode / on-chain verification / chain proof allowlist. No omni-net change. No omni-store wire change.** No Stage 12.0–12.8 schema or canonical-byte changes — `SessionStatusReport` is local infrastructure. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 13 integration tests cover the full overall-status decision tree (no-session / no-assignments / in-progress / complete-partials / aggregated / expired-incomplete / invalid-state via tampered assignment coordinator sig, tampered partial contributor sig, forged join, aggregate-present-but-invalid), the JSON serde round-trip, advert filtering vs `--include-expired`, and the session-id-mismatch-as-missing defense-in-depth case. 1 new omni-node clap regression test pinning the full flag set of `session-status` (defaults + every individual toggle). | `crates/omni-contributor/src/status.rs` (new); extended `crates/omni-contributor/src/error.rs` (new `StatusError` variant), `crates/omni-contributor/src/lib.rs` (module + re-exports), `crates/omni-node/src/contributor_cli.rs` (new `session-status` subcommand + three output renderers + clap regression test), `crates/omni-contributor/tests/status_report.rs` (new), `docs/stage12-contributor-protocol.md` (Stage 12.9 section) | **Complete** |
| **Phase 5 Stage 12.8** — Local pooled-session assignment planner. Builds on Stage 12.7 state-dir loaders + Stage 12.3 `WorkAssignment` publish path. New `omni_contributor::planner` module (`AssignmentPlan` v1 — local-only, **unsigned**; `ModelPlan` v1 operator-supplied work shape; `PlannerStrategy` closed enum: `SequentialLayers` / `SingleContributor` / `RoundRobin`; `plan_assignments` entry point; `plan_hash_hex` BLAKE3 integrity tag over canonical JSON). Deterministic post-filter selection: contributors sorted by `contributor_pubkey_hex` ASCII order; **no RAM-weighted ranking, no scoring, no marketplace behavior**. Eligibility filter is pass/fail (`min_available_ram_bytes` floor + optional `require_live_routing` matching peer-advert dtype + `supports_live_handoff`). Two new CLI subcommands under `omni-node operator contributor`: `plan-session-assignments` (read-only; loads + re-verifies session/joins/optional peer-adverts from `--contributor-state-dir`, runs the planner, emits an `AssignmentPlan` JSON — no SNIP, no mesh, no signing) and `assign-session-plan` (reads the plan, re-fetches + re-verifies session via `--session-snip-root`, re-derives `plan_hash`, publishes each entry as a normal Stage 12.3 `WorkAssignment` via the new extracted `publish_one_signed_assignment` helper; `--dry-run` opts out of SNIP/mesh; `--contributor-state-dir` mirrors published assignments). State-dir loaders are parse-only (Stage 12.7 trust boundary preserved); both CLI commands re-run `verify_execution_session` + `verify_contributor_join` + `verify_peer_advertisement_body` before the planner sees the artifacts, and the planner library entry re-verifies internally too as defense-in-depth. Stage 12.3 `assign-work` keeps working bit-for-bit (publish helper extraction is a refactor; behavior unchanged). **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid / pricing logic. No proof mode / on-chain verification / chain proof allowlist. No omni-net change. No omni-store wire change.** No Stage 12.0–12.7 schema or canonical-byte changes. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 16 planner unit tests (strategy correctness for all three strategies, equal-split contiguous non-overlapping ranges, model-plan-driven shape overrides layer-count, deterministic-under-shuffle, plan_hash serde round-trip stability, RAM-floor pass/fail filtering, live-routing dtype-mismatch + expired-advert filtering + match-passes, model-plan validation rejects inverted layers / out-of-order stage_index / unknown schema_version, max-assignments truncation, missing-inputs-is-typed-error). 7 integration tests (state-dir → planner round-trip, caller-side filter drops forged join, planner internal defense-in-depth catches forged join, state-dir filters expired advert when live-routing required, empty-joins yields typed error, plan_hash survives disk write/read, planned assignments after signing pass Stage 12.3 `verify_work_assignment`). | `crates/omni-contributor/src/planner.rs` (new); extended `crates/omni-contributor/src/error.rs` (new `PlannerError` variant), `crates/omni-contributor/src/lib.rs` (module + re-exports), `crates/omni-node/src/contributor_cli.rs` (2 new subcommands + extracted `publish_one_signed_assignment` helper used by both `assign-work` and `assign-session-plan`), `crates/omni-contributor/tests/planner_unit.rs` + `tests/planner_resume_integration.rs` (new), `docs/stage12-contributor-protocol.md` (Stage 12.8 section) | **Complete** |
| **Phase 5 Stage 12.7** — Local contributor workflow state persistence + restart resume. Pairs with Stage 12.6 persistent mesh identity. New `omni_contributor::state::ContributorStateStore` (JSON tree + marker files, atomic tempfile+rename writes, `state_version: 1` pinned in `meta/state_version.json` with forward-incompat refusal of future versions). Layout: `seen/{posted-jobs,network-job-announcements,network-result-announcements,sessions,joins,assignments,partials,aggregates,peer-adverts}/...` for cross-restart dedup; `verified/sessions/<id>/{session,joins/*,assignments/*,partials/*,aggregated,peer-adverts/*}.json` for cached envelopes (bit-identical to Stage 12.3 `watch-sessions --out-dir` + Stage 12.5 `watch-peer-adverts --out-dir` layouts, enabling gradual migration); `results/{contributor-results/*,result-links/*}.json` for run output mirrors. Auto-prune on open removes sessions / peer-adverts past `expires_at_utc` with cascade through joins/assignments/partials/aggregates + matching seen markers; `--no-prune-state-on-start` disables for forensic re-runs. Two new CLI flags (`--contributor-state-dir`, `--no-prune-state-on-start`) wired onto every restart-sensitive subcommand (`watch-jobs`, `watch-network-jobs`, `watch-network-results`, `watch-sessions`, `watch-peer-adverts`, `run-assignment`). When supplied, each watcher loads cross-restart seen markers via `is_seen(StateNamespace::*)` BEFORE in-memory dedup, dual-writes verified bodies into the state-dir, and lays down `mark_seen` on success. For `run-assignment --resolve-downstream-peer-from-session`, the state-dir replaces `--peer-advert-dir` + `--joins-dir`; mixing flags errors with `StateError::AmbiguousSource { legacy_flag }`. Absent the flag, every command behaves bit-identically to pre-12.7. **No chain wire / Stage 7b tx / SUM Chain RPC / `InferenceAttestationDigest` change. No payment / reward / staking / slashing. No marketplace / auction / bid logic. No proof mode / on-chain verification / chain proof allowlist. No omni-net change.** No Stage 12.0–12.6 schema or canonical-byte changes — the state-dir is local infrastructure. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes. 15 unit tests cover open-creates-layout / refuses-future-version / reuses-existing-meta / `mark_seen`+`is_seen` round-trip + idempotency / verified-JSON round-trip + atomic-overwrite-no-tempfile-leak / list-walks-tree / prune-removes-expired-sessions / prune-removes-expired-peer-adverts / prune-cascade-drops-seen-markers / auto-prune-on/off. 4 integration tests cover state-dir-layout-matches-watch-sessions / peer-advert-dual-write-survives-reopen / routing-cache-rebuilt-from-state-only / cross-restart-posted-job-dedup. | `crates/omni-contributor/src/state.rs` (new); extended `crates/omni-contributor/src/error.rs` (new `StateError` variant), `crates/omni-contributor/src/lib.rs` (module + re-exports), `crates/omni-contributor/src/watch.rs` (`WatchOptions.state_store`), `crates/omni-node/src/contributor_cli.rs` (6 subcommands gain `--contributor-state-dir` + `--no-prune-state-on-start`; `run_assignment` resolver branches; handlers dual-write), `crates/omni-contributor/tests/state_store_unit.rs` + `tests/state_resume_integration.rs` (new), `docs/stage12-contributor-protocol.md` (Stage 12.7 section) | **Complete** |
| **Phase 5 Stage 12.6** — Persistent libp2p mesh identity. Fixes the operational gap exposed by Stage 12.5: pre-12.6 `OmniNet::new` calls `SwarmBuilder::with_new_identity()`, so every `omni-node` restart kills every previously published peer advertisement no matter how much of its ≤24h freshness window was left. 12.6 adds `NetConfig.identity: NetIdentity` (default `Ephemeral`; new `KeypairProtobufBytes(_)` variant) + a new `omni-net::identity` module with `load_or_create_keypair_file_bytes(&Path) → Result<Vec<u8>, IdentityError>` (auto-create at `0o600` on Unix; refuse malformed existing files; refuse world/group-readable files on Unix). New `--net-identity-file <path>` CLI flag plumbed through every contributor subcommand that opens `OmniNet` (`announce-job`, `watch-network-jobs`, `announce-result`, `watch-network-results`, `open-session`, `join-session`, `assign-work`, `run-assignment`, `aggregate-session`, `watch-sessions`, `send-handoff`, `advertise-peer`, `watch-peer-adverts`). Key serialization uses libp2p protobuf encoding (interoperable with go-libp2p / kubo identity files). Failure posture is strict: no silent fallback to ephemeral when an existing file is malformed; no load when Unix perms are too broad. Roles stay disjoint — the mesh identity file is NOT a contributor seed; rustdoc + Stage 12.6 docs spell this out. **No chain wire / payment / marketplace / proof-mode / on-chain change.** No Stage 12.0–12.5 schema or canonical-byte changes. `advertise-peer` continues deriving PeerId from `OmniNet::local_peer_id()` — now stable across restart when `--net-identity-file` is supplied. Default `omni-node` tree still pulls zero halo2/prover/framework runtimes; `libp2p::identity` was already transitively present via `omni-net`. 7 new identity unit tests (auto-create / `0o600` / refuse-broader-perms / refuse-malformed / round-trip / two-distinct-files / non-regular-file) + 3 new top-level `OmniNet` tests (persistent identity yields stable PeerId across instances / two different identity files give different PeerIds / `OmniNet::new` rejects malformed identity bytes). | `crates/omni-net/src/identity.rs` (new); extended `crates/omni-types/src/config.rs`, `crates/omni-net/src/lib.rs` + `swarm.rs`, `crates/omni-net/Cargo.toml` (tempfile dev-dep), `crates/omni-node/src/contributor_cli.rs` (13 subcommands gain `--net-identity-file`), `docs/stage12-contributor-protocol.md` (Stage 12.6 section) | **Complete** |
...
| **InferenceSettlement subprotocol — read surface (issues #78 / #83 / #84 / #80 / #85)**: dormancy detection, session / claim / dispute / verifier registry / consistency-group read RPCs; multi-verifier composed views; identity mapping + bond precheck helpers; `event=settlement_*` observability markers. Activation-gated on chain params. | `omni-sumchain::settlement`, `omni-node::settlement_cli`, `omni-node::settlement_signer` | **Complete (activation-gated)** |
| **InferenceSettlement subprotocol — verifier self-claim write path (issue #87)**: `operator settlement claim` behind `--features settlement-submit`; chain-authoritative tx construction via `omninode_buildClaimInferenceReward`; local dormancy / attestation / authority / maturity / bond prechecks; identity vs. signing-seed retention split preserved. | `omni-sumchain::settlement_submit`, `omni-node::settlement_cli` (Claim variant) | **Complete (activation-gated)** |
| **InferenceSettlement subprotocol — dispute open + resolve write path (issue #81)**: `operator settlement dispute { open, resolve }` behind `--features settlement-dispute`; chain-authoritative tx construction via `omninode_buildOpenInferenceDispute` / `omninode_buildResolveInferenceDispute`; validator-quorum approvals JSON (hex) passed through as-is; local funder / open-status / maturity / duplicate-dispute / dispute-Open-state / non-empty-approvals prechecks. | `omni-sumchain::settlement_dispute`, `omni-node::settlement_cli` (Dispute variant) | **Complete (activation-gated)** |
| Phase 5 Stage 8+ — SUM Chain **tokenomics + chain-side semantic proof verification** (staking beyond settlement escrow / on-chain zkML / Halo2 chain-side verify / AI-semantic-correctness enforcement) — **still gated on Stage 11d.3 sign-off + dedicated chain-team plan.** Settlement + dispute have shipped (activation-gated); this row is the residual future work. | `contracts/` | Planned |
...
4. `cargo run -p omni-node --features submit -- operator smoke --expect-chain-id 1 --registry-path <P> --attestation-json ./first-attestation.json --allow-submit --allow-mainnet-submit [--confirm-timeout-secs N]`. **Stage 9a:** the `submit` cargo feature is default-off; the binary used in steps 1–3 (read-only) does not need it, but `smoke` does. **Timeout guidance (empirically corrected, Stage 8c):** the first mainnet smoke (2026-05-19, see [docs/mainnet-smoke-audit.md](mainnet-smoke-audit.md)) finalized in ~**11 s** / ~1 block past submit, so the default `--confirm-timeout-secs 60` is comfortably sufficient on SUM Chain mainnet — no need to inflate it. No block time is hardcoded; raise it only if your endpoint or network conditions warrant. Record the **tx hash**.
...
> **Mainnet smoke: PASS (2026-05-19, SUM Chain `chain_id=1`).** First real OmniNode attestation finalized on mainnet — see the operational run record in [docs/mainnet-smoke-audit.md](mainnet-smoke-audit.md). README stays the feature doc; the audit note holds the exact tx hash / heights / finality timing.
...
**Crate:** `omni-node` | **Depends on:** Stage 8b + the 2026-05-19 mainnet smoke ([docs/mainnet-smoke-audit.md](mainnet-smoke-audit.md)) | zero edits outside `crates/omni-node/`, no new deps
...
**Audit note immutability:** [docs/mainnet-smoke-audit.md](mainnet-smoke-audit.md) records the 2026-05-19 run as a frozen historical record; Stage 8c added only a short "follow-up hardening" pointer at its foot — the smoke facts themselves are unedited.
...
**Crates:** `omni-node`, `omni-sumchain` (additive feature) | **Depends on:** Stage 8c + the 2026-05-19 mainnet smoke ([docs/mainnet-smoke-audit.md](mainnet-smoke-audit.md)) | no new external crates; zero protocol changes; Stage 6 fixture + Stage 7b tx-construction byte-stable
...
**3. Runbook.** A canonical operator runbook lives at [docs/operator-runbook.md](operator-runbook.md): build matrix, daily preflight, submit / loop / monitor sequencing, registry inspection, recovery after process interruption, backup / rotation, what-to-report-on-incident fields, `RUST_LOG` guidance, the stable tracing-field markers to grep, and a **docs-only systemd unit sample** (no installer, no service-manager code in the binary). README's Stage 8b runbook step 4 was updated to use `cargo run -p omni-node --features submit -- operator smoke …` (read-only steps 1–3 stay plain).

**Audit-note immutability:** the 2026-05-19 [mainnet smoke audit](mainnet-smoke-audit.md) remains historical. Stage 9a's hardening is described here in the README, not by editing the audit's recorded facts.
...
**Distribution discoverability (Q5):** a short "Choosing your build" pointer table lives at the top of this README (linking to the runbook); the full table + `--help` discoverability notes (e.g., `operator smoke` is **absent** from `--help` without the feature; clap reports "unrecognized subcommand 'smoke'" rather than a runtime stub) live in [`docs/operator-runbook.md`](operator-runbook.md) § 1.
...
**2. Stable `event=` log markers.** Three new structured tracing markers join the existing free-form lines, pinned by [`docs/operator-runbook.md` §7a](operator-runbook.md):
...
**4. Release-readiness docs ([runbook §1a "Capturing `--help` for verification"](operator-runbook.md) + [new §14](operator-runbook.md)).** Documentation-only, no automation. Covers:
...
**Crates touched:** new [`crates/omni-zkml/src/proof.rs`](../crates/omni-zkml/src/proof.rs) (traits + mock impls + orchestrator), [`crates/omni-zkml/src/error.rs`](../crates/omni-zkml/src/error.rs) (typed errors), [`crates/omni-zkml/tests/proof_pipeline_vectors.rs`](../crates/omni-zkml/tests/proof_pipeline_vectors.rs) + [fixture JSON](../crates/omni-zkml/tests/fixtures/proof_pipeline_vectors.json), [`crates/omni-node/src/operator.rs`](../crates/omni-node/src/operator.rs) (new `build_mock_v1_attestation` + mainnet guard + 2 new tests), [`crates/omni-node/Cargo.toml`](../crates/omni-node/Cargo.toml) (adds `blake3` dep), [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) (new `stage11a-fixture-check` job), [`docs/operator-runbook.md`](operator-runbook.md) (§4 + §11a + §14 updates), README. | **Depends on:** Stage 9c.1 chain-produced fixture + Stage 10a operator observability + Phase 5 RC audit `d695e41`. | **Zero protocol changes; zero new chain RPCs; Stage 6 chain-wire + fixture and Stage 7b tx construction byte-stable.**

Replaces the pre-Stage-11a placeholder proof surface — where [`crates/omni-node/src/operator.rs`](../crates/omni-node/src/operator.rs)'s `synth_attestation` fabricated `model_hash="a".repeat(64)` / `manifest_snip_root=[0x11;32]` / `response_hash="b".repeat(64)` / `proof_snip_root=[0x22;32]` for every commitment field (the bytes that landed on chain in the 2026-05-19 mainnet smoke, see [`docs/mainnet-smoke-audit.md`](mainnet-smoke-audit.md) and [`docs/phase5-rc-audit.md`](phase5-rc-audit.md)) — with a real backend-trait-driven pipeline. Stage 11a is **trait + hermetic fixture harness first**: the real proof system (ezkl / risc0 / sp1 / …) is deliberately deferred to Stage 11b so we don't ship heavyweight prover dependencies before the trait shape has been validated end-to-end.
...
- **No edits to** [`docs/mainnet-smoke-audit.md`](mainnet-smoke-audit.md) **or** [`docs/phase5-rc-audit.md`](phase5-rc-audit.md) (both immutable).
...
| On-Chain Verification | Future work — not in Stage 11d. The Stage 11d.1 allowlist schema is shaped so a future on-chain verifier is a refactor, not a schema change. See [docs/mainnet-eligibility-criteria.md §5](mainnet-eligibility-criteria.md). |
...
│   │
│   ├── omni-sumchain/                  # Phase 5 Stage 7a + 7b + 9a + 9c + settlement track: SUM Chain adapter (read/query default-on; submit + settlement-submit + settlement-dispute pull public chain primitives from crates.io v0.2.0 behind those features, default-off; settlement-read is dependency-neutral)
...
│   │       ├── parity_vendored_primitives.rs # 5 byte-equivalence tests: Stage 6 local digest/tx-data + bs58 address derivation == public chain types (sumchain-primitives v0.2.0; TxPayload::InferenceAttestation ordinal 21 preserved from v0.1.0 per the append-only invariant) under bincode 1.3, plus Stage 9c TransactionV2 signing-hash + SignedTransaction hex-roundtrip stability
...
| `alloy` | n/a | Originally planned for EVM interaction | **Not depended upon.** SUM Chain is not EVM; existing chain integration uses `sumchain-primitives` / `sumchain-crypto` v0.2.0 (bumped from v0.1.0 in issue #87 to pick up the InferenceSettlement variant on `TxPayload`; append-only invariant preserved). |
