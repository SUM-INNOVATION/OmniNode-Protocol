...
mod tests {
    ...
    /// Helper for the default form. Pre-fills
    ...
    fn state_integrity_diff_flag_parse_smoke() {
        ...
        // `--baseline` is required for state-integrity-diff.
        let no_baseline = TestRoot::try_parse_from([
            "omni-node",
            "state-integrity-diff",
            "--current",
        ...
        assert!(
            no_baseline.is_err(),
            "clap must require --baseline for state-integrity-diff"
        );
    }
    ...
    mod stage_14_3_external_runner_sidecar_proof {
        ...
        // the Stage 14.3 acceptance bar. Uses the standard
        // `#[cfg(unix)]` + `set_permissions(0o755)` pattern. CI
        // runs on Linux.
...
fn run_state_integrity(args: StateIntegrityArgs) -> Result<()> {
    ...
    if let Some(baseline_path) = args.baseline.as_deref() {
        let baseline = load_baseline_report(baseline_path)?;
...
/// `run_state_integrity`. Called when `args.baseline` is set.
/// The caller has already loaded the baseline JSON to a
...
fn run_state_integrity_diff(args: StateIntegrityDiffArgs) -> Result<()> {
    ...
    let baseline = load_baseline_report(&args.baseline)?;
...
/// Stage 12.19 — load a baseline v1 `StateIntegrityReport` JSON
